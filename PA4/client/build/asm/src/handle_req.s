	.file	"handle_req.c"
	.text
.Ltext0:
	.file 0 "/home/parth/Work/All_data/university/Network System/Assignments/PA4/client" "src/handle_req.c"
	.globl	str2md5
	.type	str2md5, @function
str2md5:
.LFB320:
	.file 1 "src/handle_req.c"
	.loc 1 54 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$80, %rsp
	movq	%rdi, -72(%rbp)
	movl	%esi, -76(%rbp)
	.loc 1 54 1
	movq	%fs:40, %rax
	movq	%rax, -8(%rbp)
	xorl	%eax, %eax
	.loc 1 55 27
	call	EVP_MD_CTX_new@PLT
	movq	%rax, -48(%rbp)
	.loc 1 56 24
	call	EVP_md5@PLT
	movq	%rax, -40(%rbp)
	.loc 1 57 5
	movq	-40(%rbp), %rcx
	movq	-48(%rbp), %rax
	movl	$0, %edx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	EVP_DigestInit_ex@PLT
	.loc 1 61 11
	jmp	.L2
.L5:
	.loc 1 63 12
	cmpl	$512, -76(%rbp)
	jle	.L3
	.loc 1 65 13
	movq	-72(%rbp), %rcx
	movq	-48(%rbp), %rax
	movl	$512, %edx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	EVP_DigestUpdate@PLT
	jmp	.L4
.L3:
	.loc 1 69 13
	movl	-76(%rbp), %eax
	movslq	%eax, %rdx
	movq	-72(%rbp), %rcx
	movq	-48(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	EVP_DigestUpdate@PLT
.L4:
	.loc 1 71 16
	subl	$512, -76(%rbp)
	.loc 1 72 13
	addq	$512, -72(%rbp)
.L2:
	.loc 1 61 19
	cmpl	$0, -76(%rbp)
	jg	.L5
	.loc 1 74 5
	leaq	-60(%rbp), %rdx
	leaq	-32(%rbp), %rcx
	movq	-48(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	EVP_DigestFinal_ex@PLT
	.loc 1 75 5
	movq	-48(%rbp), %rax
	movq	%rax, %rdi
	call	EVP_MD_CTX_free@PLT
	.loc 1 76 14
	movl	$0, -56(%rbp)
.LBB2:
	.loc 1 77 14
	movl	$0, -52(%rbp)
	.loc 1 77 5
	jmp	.L6
.L7:
	.loc 1 79 23
	movl	-52(%rbp), %eax
	cltq
	movzbl	-32(%rbp,%rax), %eax
	movzbl	%al, %eax
	.loc 1 79 14
	addl	%eax, -56(%rbp)
	.loc 1 77 33 discriminator 3
	addl	$1, -52(%rbp)
.L6:
	.loc 1 77 23 discriminator 1
	movl	-52(%rbp), %edx
	movl	-60(%rbp), %eax
	cmpl	%eax, %edx
	jb	.L7
.LBE2:
	.loc 1 82 12
	movl	-56(%rbp), %eax
	.loc 1 83 1
	movq	-8(%rbp), %rdx
	subq	%fs:40, %rdx
	je	.L9
	call	__stack_chk_fail@PLT
.L9:
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE320:
	.size	str2md5, .-str2md5
	.globl	get_in_addr
	.type	get_in_addr, @function
get_in_addr:
.LFB321:
	.loc 1 86 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	movq	%rdi, -8(%rbp)
	.loc 1 87 11
	movq	-8(%rbp), %rax
	movzwl	(%rax), %eax
	.loc 1 87 8
	cmpw	$2, %ax
	jne	.L11
	.loc 1 89 16
	movq	-8(%rbp), %rax
	addq	$4, %rax
	jmp	.L12
.L11:
	.loc 1 92 12
	movq	-8(%rbp), %rax
	addq	$8, %rax
.L12:
	.loc 1 93 1
	popq	%rbp
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE321:
	.size	get_in_addr, .-get_in_addr
	.globl	cleanup_current_connection
	.type	cleanup_current_connection, @function
cleanup_current_connection:
.LFB322:
	.loc 1 96 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$16, %rsp
	movq	%rdi, -8(%rbp)
	.loc 1 97 5
	movq	-8(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 98 29
	movq	-8(%rbp), %rax
	movl	$-1, (%rax)
	.loc 1 99 16
	movq	-8(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 99 8
	testq	%rax, %rax
	je	.L14
	.loc 1 100 21
	movq	-8(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 100 9
	movq	%rax, %rdi
	call	free@PLT
.L14:
	.loc 1 101 24
	movq	-8(%rbp), %rax
	movq	$0, 16(%rax)
	.loc 1 102 16
	movq	-8(%rbp), %rax
	movq	8(%rax), %rax
	.loc 1 102 8
	testq	%rax, %rax
	je	.L15
	.loc 1 103 21
	movq	-8(%rbp), %rax
	movq	8(%rax), %rax
	.loc 1 103 9
	movq	%rax, %rdi
	call	free@PLT
.L15:
	.loc 1 104 26
	movq	-8(%rbp), %rax
	movq	$0, 8(%rax)
	.loc 1 105 1
	nop
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE322:
	.size	cleanup_current_connection, .-cleanup_current_connection
	.globl	cleanup_connection
	.type	cleanup_connection, @function
cleanup_connection:
.LFB323:
	.loc 1 108 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$32, %rsp
	movq	%rdi, -24(%rbp)
	.loc 1 109 22
	movq	-24(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -16(%rbp)
	.loc 1 110 11
	jmp	.L17
.L18:
.LBB3:
	.loc 1 112 26
	movq	-16(%rbp), %rax
	movq	%rax, -8(%rbp)
	.loc 1 113 17
	movq	-16(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -16(%rbp)
	.loc 1 114 9
	movq	-8(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
.L17:
.LBE3:
	.loc 1 110 12
	cmpq	$0, -16(%rbp)
	jne	.L18
	.loc 1 116 37
	movq	-24(%rbp), %rax
	movl	$0, 60(%rax)
	.loc 1 117 27
	movq	-24(%rbp), %rax
	movl	$0, 56(%rax)
	.loc 1 118 11
	movq	-24(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 118 8
	testq	%rax, %rax
	je	.L19
	.loc 1 119 16
	movq	-24(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 119 9
	movq	%rax, %rdi
	call	free@PLT
.L19:
	.loc 1 120 25
	movq	-24(%rbp), %rax
	movq	$0, 48(%rax)
	.loc 1 121 1
	nop
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE323:
	.size	cleanup_connection, .-cleanup_connection
	.section	.rodata
	.align 8
.LC0:
	.string	"\033[31m[-] Port Value < 1024 ! keep port value higher than 1024 \n\033[0m"
	.align 8
.LC1:
	.string	"\033[31m[-] ERROR: DNS resolution failed - %s\n\033[0m"
	.align 8
.LC2:
	.string	"\033[31m[-] ERROR: Cannot create socket for server (%s:%s)\n\033[0m"
	.align 8
.LC3:
	.string	"\033[31m\n[-] CONNECTION FAILED: Server %d (%s:%s)\n\033[0m"
	.align 8
.LC4:
	.string	"\033[31m    Error: %s (errno: %d)\n\033[0m"
	.align 8
.LC5:
	.string	"\033[31m\n[-] ERROR: Connection attempt exhausted all available addresses (thread: %d, errno: %d)\n\033[0m"
	.align 8
.LC6:
	.string	"\033[32m\n[+] CONNECTION ESTABLISHED: Server %d\n\033[0m"
.LC7:
	.string	"\033[32m    Address: %s:%s\n\033[0m"
	.text
	.globl	connect_server
	.type	connect_server, @function
connect_server:
.LFB324:
	.loc 1 124 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	pushq	%rbx
	subq	$168, %rsp
	.cfi_offset 3, -24
	movq	%rdi, -152(%rbp)
	movq	%rsi, -160(%rbp)
	movl	%edx, -164(%rbp)
	.loc 1 124 1
	movq	%fs:40, %rax
	movq	%rax, -24(%rbp)
	xorl	%eax, %eax
	.loc 1 126 9
	movl	$1, -140(%rbp)
	.loc 1 128 7
	movq	-152(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 128 24
	movl	-164(%rbp), %edx
	movslq	%edx, %rdx
	salq	$2, %rdx
	addq	%rdx, %rax
	.loc 1 128 39
	movl	$-1, (%rax)
	.loc 1 129 29
	movq	-160(%rbp), %rax
	movl	$-1, (%rax)
	.loc 1 132 5
	leaq	-128(%rbp), %rax
	movl	$48, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 133 21
	movl	$0, -124(%rbp)
	.loc 1 134 23
	movl	$1, -120(%rbp)
	.loc 1 136 21
	movq	-160(%rbp), %rax
	movq	8(%rax), %rax
	.loc 1 136 9
	movq	%rax, %rdi
	call	atoi@PLT
	.loc 1 136 8 discriminator 1
	cmpl	$1024, %eax
	jg	.L21
	.loc 1 138 9
	movq	stderr(%rip), %rax
	movq	%rax, %rcx
	movl	$67, %edx
	movl	$1, %esi
	leaq	.LC0(%rip), %rax
	movq	%rax, %rdi
	call	fwrite@PLT
	.loc 1 139 9
	movl	$1, %edi
	call	exit@PLT
.L21:
	.loc 1 142 19
	movq	-152(%rbp), %rax
	leaq	40(%rax), %rcx
	.loc 1 142 58
	movq	-160(%rbp), %rax
	movq	8(%rax), %rsi
	.loc 1 142 38
	movq	-160(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 142 19
	leaq	-128(%rbp), %rdx
	movq	%rax, %rdi
	call	getaddrinfo@PLT
	movl	%eax, -140(%rbp)
	.loc 1 142 8 discriminator 1
	cmpl	$0, -140(%rbp)
	jns	.L22
	.loc 1 144 9
	movl	-140(%rbp), %eax
	movl	%eax, %edi
	call	gai_strerror@PLT
	movq	%rax, %rdx
	.loc 1 144 9 is_stmt 0 discriminator 1
	movq	stderr(%rip), %rax
	leaq	.LC1(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	fprintf@PLT
	.loc 1 145 16 is_stmt 1
	movl	$-1, -140(%rbp)
	.loc 1 146 9
	jmp	.L23
.L22:
	.loc 1 149 15
	movq	-152(%rbp), %rax
	movq	40(%rax), %rax
	movq	%rax, -136(%rbp)
	.loc 1 149 5
	nop
	.loc 1 149 43 discriminator 1
	cmpq	$0, -136(%rbp)
	je	.L27
	.loc 1 151 40
	movq	-136(%rbp), %rax
	movl	12(%rax), %edx
	movq	-136(%rbp), %rax
	movl	8(%rax), %ecx
	movq	-136(%rbp), %rax
	movl	4(%rax), %eax
	movl	%ecx, %esi
	movl	%eax, %edi
	call	socket@PLT
	.loc 1 151 38 discriminator 1
	movq	-160(%rbp), %rdx
	movl	%eax, (%rdx)
	.loc 1 151 21 discriminator 1
	movq	-160(%rbp), %rax
	movl	(%rax), %eax
	.loc 1 151 12 discriminator 1
	testl	%eax, %eax
	jns	.L25
	.loc 1 153 13
	movq	-160(%rbp), %rax
	movq	8(%rax), %rcx
	movq	-160(%rbp), %rax
	movq	16(%rax), %rdx
	movq	stderr(%rip), %rax
	leaq	.LC2(%rip), %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	fprintf@PLT
	.loc 1 154 20
	movl	$-1, -140(%rbp)
	.loc 1 155 13
	jmp	.L23
.L25:
	.loc 1 158 14
	movq	-136(%rbp), %rax
	movl	16(%rax), %edx
	.loc 1 158 51
	movq	-136(%rbp), %rax
	movq	24(%rax), %rax
	.loc 1 158 14
	movq	%rax, %rcx
	movq	-160(%rbp), %rax
	movl	(%rax), %eax
	movq	%rcx, %rsi
	movl	%eax, %edi
	call	connect@PLT
	.loc 1 158 12 discriminator 1
	testl	%eax, %eax
	jns	.L32
	.loc 1 160 13
	movq	-160(%rbp), %rax
	movq	8(%rax), %rcx
	movq	-160(%rbp), %rax
	movq	16(%rax), %rdx
	movl	-164(%rbp), %eax
	leal	1(%rax), %esi
	movq	stderr(%rip), %rax
	movq	%rcx, %r8
	movq	%rdx, %rcx
	movl	%esi, %edx
	leaq	.LC3(%rip), %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	fprintf@PLT
	.loc 1 161 87
	call	__errno_location@PLT
	.loc 1 161 13 discriminator 1
	movl	(%rax), %ebx
	.loc 1 161 79 discriminator 1
	call	__errno_location@PLT
	.loc 1 161 13 discriminator 2
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rdx
	.loc 1 161 13 is_stmt 0 discriminator 3
	movq	stderr(%rip), %rax
	movl	%ebx, %ecx
	leaq	.LC4(%rip), %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	fprintf@PLT
	.loc 1 162 13 is_stmt 1
	movq	-160(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 163 20
	movl	$-1, -140(%rbp)
	.loc 1 164 13
	jmp	.L23
.L32:
	.loc 1 167 9
	nop
.L27:
	.loc 1 170 8
	cmpq	$0, -136(%rbp)
	jne	.L29
	.loc 1 172 140
	call	__errno_location@PLT
	.loc 1 172 9 discriminator 1
	movl	(%rax), %ebx
	call	gettid@PLT
	movl	%eax, %edx
	.loc 1 172 9 is_stmt 0 discriminator 2
	movq	stderr(%rip), %rax
	movl	%ebx, %ecx
	leaq	.LC5(%rip), %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	fprintf@PLT
	.loc 1 173 16 is_stmt 1
	movl	$-1, -140(%rbp)
	.loc 1 174 9
	jmp	.L23
.L29:
	.loc 1 177 7
	movq	-152(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 177 24
	movl	-164(%rbp), %edx
	movslq	%edx, %rdx
	salq	$2, %rdx
	addq	%rax, %rdx
	.loc 1 177 48
	movq	-160(%rbp), %rax
	movl	(%rax), %eax
	.loc 1 177 39
	movl	%eax, (%rdx)
	.loc 1 178 7
	movq	-152(%rbp), %rax
	movl	60(%rax), %eax
	.loc 1 178 36
	leal	1(%rax), %edx
	movq	-152(%rbp), %rax
	movl	%edx, 60(%rax)
	.loc 1 181 32
	movq	-136(%rbp), %rax
	movq	24(%rax), %rax
	movq	%rax, %rdi
	call	get_in_addr
	movq	%rax, %rsi
	.loc 1 181 5 discriminator 1
	movq	-136(%rbp), %rax
	movl	4(%rax), %eax
	leaq	-80(%rbp), %rdx
	movl	$46, %ecx
	movl	%eax, %edi
	call	inet_ntop@PLT
	.loc 1 182 5
	movl	-164(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC6(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 183 5
	movq	-160(%rbp), %rax
	movq	8(%rax), %rdx
	movq	-160(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC7(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L23:
	.loc 1 189 5
	movq	-136(%rbp), %rax
	movq	%rax, %rdi
	call	freeaddrinfo@PLT
	.loc 1 190 12
	movl	-140(%rbp), %eax
	.loc 1 191 1
	movq	-24(%rbp), %rdx
	subq	%fs:40, %rdx
	je	.L31
	call	__stack_chk_fail@PLT
.L31:
	movq	-8(%rbp), %rbx
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE324:
	.size	connect_server, .-connect_server
	.section	.rodata
	.align 8
.LC8:
	.string	"\033[36m\n[*] FILE DISTRIBUTION MAPPING\n\033[0m"
	.align 8
.LC9:
	.string	"\033[36m    File hash value: %d\n\033[0m"
	.align 8
.LC10:
	.string	"\033[36m    Starting server: %d\n\n\033[0m"
	.align 8
.LC11:
	.string	"\033[36m[*] CHUNK ASSIGNMENT: Server %d\n\033[0m"
.LC12:
	.string	"\033[36m    Storing chunks: \033[0m"
.LC13:
	.string	"\033[36m%d \033[0m"
	.align 8
.LC14:
	.string	"\033[31m[-] ERROR: Send operation failed (errno: %d - %s)\n\033[0m"
	.align 8
.LC15:
	.string	"\033[31m[-] ERROR: Receive operation failed (errno: %d - %s)\n\033[0m"
	.align 8
.LC16:
	.string	"\033[33m[-] NOTICE: Connection closed by client\n\033[0m"
.LC17:
	.string	"ack\t\n0\t\n"
	.string	""
	.align 8
.LC18:
	.string	"\033[31m[-] ERROR: Received NACK from server for chunk %d\n\033[0m"
	.align 8
.LC19:
	.string	"\033[32m[+] Stored chunk %d to server %d\n\033[0m"
	.align 8
.LC20:
	.string	"\033[31m\n=========================================\n\033[0m"
	.align 8
.LC21:
	.string	"\033[31m    PUT OPERATION FAILED\n\033[0m"
.LC22:
	.string	"\033[31m    File: %s\n\033[0m"
	.align 8
.LC23:
	.string	"\033[31m    Cannot store file reliably\n\033[0m"
	.align 8
.LC24:
	.string	"\033[31m    Missing redundancy for chunk %d\n\033[0m"
	.align 8
.LC25:
	.string	"\033[31m=========================================\n\n\033[0m"
	.align 8
.LC26:
	.string	"\033[32m\n=========================================\n\033[0m"
	.align 8
.LC27:
	.string	"\033[32m    FILE UPLOAD SUCCESSFUL\n\033[0m"
.LC28:
	.string	"\033[32m    File: %s\n\033[0m"
	.align 8
.LC29:
	.string	"\033[32m    All chunks stored with redundancy\n\033[0m"
	.align 8
.LC30:
	.string	"\033[32m=========================================\n\n\033[0m"
	.text
	.globl	connect_and_put_chunks
	.type	connect_and_put_chunks, @function
connect_and_put_chunks:
.LFB325:
	.loc 1 194 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	pushq	%rbx
	leaq	-28672(%rsp), %r11
.LPSRL0:
	subq	$4096, %rsp
	orq	$0, (%rsp)
	cmpq	%r11, %rsp
	jne	.LPSRL0
	subq	$2184, %rsp
	.cfi_offset 3, -24
	movq	%rdi, -30840(%rbp)
	movq	%rsi, -30848(%rbp)
	movq	%rdx, -30856(%rbp)
	movl	%ecx, -30860(%rbp)
	.loc 1 194 1
	movq	%fs:40, %rax
	movq	%rax, -24(%rbp)
	xorl	%eax, %eax
	.loc 1 196 5
	leaq	.LC8(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 197 5
	movl	-30860(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC9(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 198 62
	movq	-30840(%rbp), %rax
	movl	56(%rax), %ebx
	.loc 1 198 58
	movl	-30860(%rbp), %eax
	cltd
	idivl	%ebx
	movl	%edx, %ecx
	movl	%ecx, %eax
	.loc 1 198 5
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC10(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 200 22
	movq	-30840(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30792(%rbp)
	.loc 1 201 9
	movl	$0, -30832(%rbp)
	.loc 1 202 36
	movq	-30840(%rbp), %rax
	movl	56(%rax), %eax
	cltq
	.loc 1 202 27
	salq	$2, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 202 25 discriminator 1
	movq	-30840(%rbp), %rax
	movq	%rdx, 48(%rax)
	.loc 1 207 5
	leaq	-30768(%rbp), %rax
	movl	$16, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 209 11
	jmp	.L34
.L50:
.LBB4:
	.loc 1 211 13
	movl	-30832(%rbp), %edx
	movq	-30792(%rbp), %rcx
	movq	-30840(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	connect_server
	.loc 1 211 12 discriminator 1
	testl	%eax, %eax
	js	.L56
	.loc 1 214 26
	movl	-30832(%rbp), %eax
	subl	-30860(%rbp), %eax
	.loc 1 214 63
	testl	%eax, %eax
	jns	.L37
	.loc 1 214 63 is_stmt 0 discriminator 1
	movq	-30840(%rbp), %rax
	movl	56(%rax), %eax
	jmp	.L38
.L37:
	.loc 1 214 63 discriminator 2
	movl	-30832(%rbp), %eax
	subl	-30860(%rbp), %eax
.L38:
	.loc 1 214 13 is_stmt 1 discriminator 4
	movl	%eax, -30828(%rbp)
	.loc 1 216 9
	movl	-30832(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC11(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 217 9
	leaq	.LC12(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBB5:
	.loc 1 218 18
	movl	$0, -30824(%rbp)
	.loc 1 218 9
	jmp	.L39
.L40:
	.loc 1 220 42
	movl	-30828(%rbp), %edx
	movl	-30824(%rbp), %eax
	leal	(%rdx,%rax), %ecx
	.loc 1 220 51
	movq	-30840(%rbp), %rax
	movl	56(%rax), %ebx
	.loc 1 220 47
	movl	%ecx, %eax
	cltd
	idivl	%ebx
	movl	%edx, %ecx
	movl	%ecx, %eax
	.loc 1 220 13
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC13(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 218 63 discriminator 3
	addl	$1, -30824(%rbp)
.L39:
	.loc 1 218 27 discriminator 1
	cmpl	$1, -30824(%rbp)
	jle	.L40
.LBE5:
	.loc 1 222 9
	movl	$10, %edi
	call	putchar@PLT
.LBB6:
	.loc 1 224 18
	movl	$0, -30820(%rbp)
	.loc 1 224 9
	jmp	.L41
.L49:
.LBB7:
	.loc 1 227 28
	movl	-30828(%rbp), %edx
	movl	-30820(%rbp), %eax
	addl	%eax, %edx
	.loc 1 227 37
	movq	-30840(%rbp), %rax
	movl	56(%rax), %ecx
	.loc 1 227 19
	movl	%edx, %eax
	cltd
	idivl	%ecx
	movl	%edx, -30828(%rbp)
	.loc 1 228 26
	movl	-30828(%rbp), %eax
	cltq
	movl	-30768(%rbp,%rax,4), %eax
	.loc 1 228 33
	leal	1(%rax), %edx
	movl	-30828(%rbp), %eax
	cltq
	movl	%edx, -30768(%rbp,%rax,4)
	.loc 1 230 30
	movb	$1, -30780(%rbp)
	movl	-30828(%rbp), %eax
	movb	%al, -30779(%rbp)
	.loc 1 233 45
	movq	-30840(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 233 36
	movq	%rax, %rdi
	call	strlen@PLT
	.loc 1 230 30
	movl	%eax, -30776(%rbp)
	.loc 1 234 43
	movl	-30828(%rbp), %eax
	cltq
	leaq	0(,%rax,4), %rdx
	movq	-30856(%rbp), %rax
	addq	%rdx, %rax
	movl	(%rax), %eax
	.loc 1 230 30
	movl	%eax, -30772(%rbp)
.LBB8:
	.loc 1 236 24
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30780(%rbp), %rsi
	movl	$0, %ecx
	movl	$12, %edx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 236 24 is_stmt 0 discriminator 1
	movl	%eax, -30812(%rbp)
	cmpl	$0, -30812(%rbp)
	jg	.L42
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 236 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 236 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 236 24
	jmp	.L36
.L42:
	.loc 1 236 24 discriminator 2
	movl	-30812(%rbp), %eax
.LBE8:
	.loc 1 236 22 is_stmt 1 discriminator 2
	movl	%eax, -30808(%rbp)
.LBB9:
	.loc 1 238 24
	movl	-30776(%rbp), %eax
	movl	%eax, %edx
	movq	-30840(%rbp), %rax
	movq	16(%rax), %rsi
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	movl	$0, %ecx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 238 24 is_stmt 0 discriminator 1
	movl	%eax, -30804(%rbp)
	cmpl	$0, -30804(%rbp)
	jg	.L44
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 238 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 238 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 238 24
	jmp	.L36
.L44:
	.loc 1 238 24 discriminator 2
	movl	-30804(%rbp), %eax
.LBE9:
	.loc 1 238 22 is_stmt 1 discriminator 2
	movl	%eax, -30808(%rbp)
.LBB10:
	.loc 1 240 24
	movl	-30828(%rbp), %eax
	cltq
	leaq	0(,%rax,4), %rdx
	movq	-30856(%rbp), %rax
	addq	%rdx, %rax
	movl	(%rax), %eax
	movslq	%eax, %rdx
	movl	-30828(%rbp), %eax
	cltq
	leaq	0(,%rax,8), %rcx
	movq	-30848(%rbp), %rax
	addq	%rcx, %rax
	movq	(%rax), %rsi
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	movl	$0, %ecx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 240 24 is_stmt 0 discriminator 1
	movl	%eax, -30800(%rbp)
	cmpl	$0, -30800(%rbp)
	jg	.L45
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 240 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 240 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 240 24
	jmp	.L36
.L45:
	.loc 1 240 24 discriminator 2
	movl	-30800(%rbp), %eax
.LBE10:
	.loc 1 240 22 is_stmt 1 discriminator 2
	movl	%eax, -30808(%rbp)
	.loc 1 242 13
	leaq	-30752(%rbp), %rax
	movl	$30720, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
.LBB11:
	.loc 1 243 24
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30752(%rbp), %rsi
	movl	$0, %ecx
	movl	$30720, %edx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 243 24 is_stmt 0 discriminator 1
	movl	%eax, -30796(%rbp)
	cmpl	$0, -30796(%rbp)
	jns	.L46
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 243 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 243 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC15(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 243 24
	jmp	.L36
.L46:
	.loc 1 243 24 discriminator 2
	cmpl	$0, -30796(%rbp)
	jne	.L47
	.loc 1 243 24 discriminator 3
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 243 24 discriminator 1
	leaq	.LC16(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 243 24
	jmp	.L36
.L47:
	.loc 1 243 24 discriminator 4
	movl	-30796(%rbp), %eax
.LBE11:
	.loc 1 243 22 is_stmt 1 discriminator 4
	movl	%eax, -30808(%rbp)
	.loc 1 245 17
	leaq	-30752(%rbp), %rax
	movl	$7, %edx
	leaq	.LC17(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 245 16 discriminator 1
	testl	%eax, %eax
	je	.L48
	.loc 1 247 17
	movl	-30828(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC18(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 248 17
	jmp	.L36
.L48:
	.loc 1 251 17
	movl	-30832(%rbp), %eax
	leal	1(%rax), %edx
	movl	-30828(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC19(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBE7:
	.loc 1 224 63 discriminator 2
	addl	$1, -30820(%rbp)
.L41:
	.loc 1 224 27 discriminator 1
	cmpl	$1, -30820(%rbp)
	jle	.L49
.LBE6:
	.loc 1 257 5
	jmp	.L36
.L56:
	.loc 1 212 13
	nop
.L36:
	.loc 1 258 9
	movq	-30792(%rbp), %rax
	movq	%rax, %rdi
	call	cleanup_current_connection
	.loc 1 259 10
	addl	$1, -30832(%rbp)
	.loc 1 260 17
	movq	-30792(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30792(%rbp)
.L34:
.LBE4:
	.loc 1 209 12
	cmpq	$0, -30792(%rbp)
	jne	.L50
.LBB12:
	.loc 1 263 14
	movl	$0, -30816(%rbp)
	.loc 1 263 5
	jmp	.L51
.L54:
	.loc 1 265 26
	movl	-30816(%rbp), %eax
	cltq
	movl	-30768(%rbp,%rax,4), %eax
	.loc 1 265 12
	testl	%eax, %eax
	jg	.L52
	.loc 1 267 13
	leaq	.LC20(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 268 13
	leaq	.LC21(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 269 13
	movq	-30840(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC22(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 270 13
	leaq	.LC23(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 271 13
	movl	-30816(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC24(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 272 13
	leaq	.LC25(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L33
.L52:
	.loc 1 263 43 discriminator 2
	addl	$1, -30816(%rbp)
.L51:
	.loc 1 263 23 discriminator 1
	cmpl	$3, -30816(%rbp)
	jle	.L54
.LBE12:
	.loc 1 277 5
	movq	-30840(%rbp), %rax
	movq	%rax, %rdi
	call	cleanup_connection
	.loc 1 280 5
	leaq	.LC26(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 281 5
	leaq	.LC27(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 282 5
	movq	-30840(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC28(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 283 5
	leaq	.LC29(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 284 5
	leaq	.LC30(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L33:
	.loc 1 285 1
	movq	-24(%rbp), %rax
	subq	%fs:40, %rax
	je	.L55
	call	__stack_chk_fail@PLT
.L55:
	movq	-8(%rbp), %rbx
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE325:
	.size	connect_and_put_chunks, .-connect_and_put_chunks
	.section	.rodata
.LC31:
	.string	"rb"
	.align 8
.LC32:
	.string	"\033[31m[-] Error opening file %d \n\033[0m"
	.align 8
.LC33:
	.string	"\033[36m\n[*] FILE PREPARATION\n\033[0m"
.LC34:
	.string	"\033[36m    Filename: %s\n\033[0m"
	.align 8
.LC35:
	.string	"\033[36m    Total size: %d bytes\n\033[0m"
	.align 8
.LC36:
	.string	"\033[36m    Chunking into %d parts\n\n\033[0m"
	.align 8
.LC37:
	.string	"\033[36m[*] CHUNK CREATED: %d of %d\n\033[0m"
.LC38:
	.string	"\033[36m    Size: %d bytes\n\n\033[0m"
	.text
	.globl	put_file
	.type	put_file, @function
put_file:
.LFB326:
	.loc 1 290 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	addq	$-128, %rsp
	movq	%rdi, -120(%rbp)
	.loc 1 290 1
	movq	%fs:40, %rax
	movq	%rax, -8(%rbp)
	xorl	%eax, %eax
	.loc 1 291 24
	movq	-120(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 291 16
	leaq	.LC31(%rip), %rdx
	movq	%rdx, %rsi
	movq	%rax, %rdi
	call	fopen@PLT
	movq	%rax, -80(%rbp)
	.loc 1 292 8
	cmpq	$0, -80(%rbp)
	jne	.L58
	.loc 1 294 67
	call	__errno_location@PLT
	.loc 1 294 9 discriminator 1
	movl	(%rax), %edx
	movq	stderr(%rip), %rax
	leaq	.LC32(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	fprintf@PLT
	jmp	.L57
.L58:
	.loc 1 297 52
	movq	-120(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 297 43
	movq	%rax, %rdi
	call	strlen@PLT
	.loc 1 297 21 discriminator 1
	movl	%eax, %edx
	movq	-120(%rbp), %rax
	movq	16(%rax), %rax
	movl	%edx, %esi
	movq	%rax, %rdi
	call	str2md5
	movl	%eax, -88(%rbp)
	.loc 1 299 5
	movq	-80(%rbp), %rax
	movl	$2, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	fseek@PLT
	.loc 1 300 16
	movq	-80(%rbp), %rax
	movq	%rax, %rdi
	call	ftell@PLT
	.loc 1 300 9 discriminator 1
	movl	%eax, -84(%rbp)
	.loc 1 301 5
	movq	-80(%rbp), %rax
	movl	$0, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	fseek@PLT
	.loc 1 305 9
	movl	$0, -100(%rbp)
	.loc 1 307 5
	leaq	.LC33(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 308 5
	movq	-120(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC34(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 309 5
	movl	-84(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC35(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 310 5
	movl	$4, %esi
	leaq	.LC36(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBB13:
	.loc 1 312 14
	movl	$0, -96(%rbp)
	.loc 1 312 5
	jmp	.L60
.L63:
.LBB14:
	.loc 1 315 12
	cmpl	$3, -96(%rbp)
	jne	.L61
	.loc 1 316 24
	movl	-84(%rbp), %eax
	subl	-100(%rbp), %eax
	movl	%eax, -92(%rbp)
	jmp	.L62
.L61:
	.loc 1 318 24
	movl	-84(%rbp), %eax
	leal	3(%rax), %edx
	testl	%eax, %eax
	cmovs	%edx, %eax
	sarl	$2, %eax
	movl	%eax, -92(%rbp)
.L62:
	.loc 1 319 36
	movl	-92(%rbp), %eax
	addl	%eax, -100(%rbp)
	.loc 1 321 24
	movl	-96(%rbp), %eax
	cltq
	movl	-92(%rbp), %edx
	movl	%edx, -64(%rbp,%rax,4)
	.loc 1 323 23
	movl	-92(%rbp), %eax
	cltq
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, -72(%rbp)
	.loc 1 324 19
	movl	-96(%rbp), %eax
	cltq
	movq	-72(%rbp), %rdx
	movq	%rdx, -48(%rbp,%rax,8)
	.loc 1 325 9
	movl	-92(%rbp), %eax
	movslq	%eax, %rdx
	movq	-80(%rbp), %rcx
	movq	-72(%rbp), %rax
	movl	$1, %esi
	movq	%rax, %rdi
	call	fread@PLT
	.loc 1 327 9
	movl	-96(%rbp), %eax
	addl	$1, %eax
	movl	$4, %edx
	movl	%eax, %esi
	leaq	.LC37(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 328 9
	movl	-92(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC38(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBE14:
	.loc 1 312 43 discriminator 2
	addl	$1, -96(%rbp)
.L60:
	.loc 1 312 23 discriminator 1
	cmpl	$3, -96(%rbp)
	jle	.L63
.LBE13:
	.loc 1 332 62
	movq	-120(%rbp), %rax
	movl	56(%rax), %eax
	movl	%eax, %ecx
	.loc 1 332 58
	movl	-88(%rbp), %eax
	movl	$0, %edx
	divl	%ecx
	movl	%edx, %eax
	.loc 1 332 5
	movl	%eax, %ecx
	leaq	-64(%rbp), %rdx
	leaq	-48(%rbp), %rsi
	movq	-120(%rbp), %rax
	movq	%rax, %rdi
	call	connect_and_put_chunks
.L57:
	.loc 1 333 1
	movq	-8(%rbp), %rax
	subq	%fs:40, %rax
	je	.L64
	call	__stack_chk_fail@PLT
.L64:
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE326:
	.size	put_file, .-put_file
	.section	.rodata
	.align 8
.LC39:
	.string	"\033[32m[+] CONNECTION ESTABLISHED: Server %d is online\n\033[0m"
	.align 8
.LC40:
	.string	"\033[36m[*] REQUESTING CHUNKS: Server %d should have chunks \033[0m"
.LC41:
	.string	"%d "
	.align 8
.LC42:
	.string	"\033[33m    [!] Skipping chunk %d (already downloaded)\n\033[0m"
	.align 8
.LC43:
	.string	"\033[36m    [*] Requesting chunk %d from server %d\n\033[0m"
	.align 8
.LC44:
	.string	"\033[31m    [-] Server %d does not have chunk %d\n\033[0m"
	.align 8
.LC45:
	.string	"\033[32m    [+] Server has chunk %d, downloading...\n\033[0m"
	.align 8
.LC46:
	.string	"\033[36m    [*] Chunk %d size: %d bytes\n\033[0m"
	.align 8
.LC47:
	.string	"\033[31m    [-] ERROR: Memory allocation failed for chunk %d\n\033[0m"
	.align 8
.LC49:
	.string	"\033[36m    [*] Downloaded %d/%d bytes (%.1f%%)\n\033[0m"
	.align 8
.LC50:
	.string	"\033[32m    [+] CHUNK %d DOWNLOADED SUCCESSFULLY (%d bytes)\n\033[0m"
	.align 8
.LC51:
	.string	"\033[31m    [-] ERROR: Failed to download chunk %d\n\033[0m"
	.align 8
.LC52:
	.string	"\033[36m\n=========================================\n\033[0m"
	.align 8
.LC53:
	.string	"\033[36m    DOWNLOAD SUMMARY: %s\n\033[0m"
	.align 8
.LC54:
	.string	"\033[36m=========================================\n\n\033[0m"
	.align 8
.LC55:
	.string	"Server status: %d/%d available\n"
	.align 8
.LC56:
	.string	"Chunks status: %d/%d downloaded\n\n"
.LC57:
	.string	"Chunk breakdown:"
	.align 8
.LC58:
	.string	"\033[32m  [\342\234\223] Chunk %d: %d bytes\n\033[0m"
	.align 8
.LC59:
	.string	"\033[31m  [\342\234\227] Chunk %d: MISSING\n\033[0m"
	.align 8
.LC60:
	.string	"\033[31m[!] CANNOT RECONSTRUCT: Missing chunk %d\n\033[0m"
	.align 8
.LC61:
	.string	"\033[31m    DOWNLOAD FAILED: %s\n\033[0m"
	.align 8
.LC62:
	.string	"\033[31m    Reason: Missing chunks prevent reconstruction\n\033[0m"
	.align 8
.LC63:
	.string	"\033[31m    Required: %d/%d chunks\n\033[0m"
	.align 8
.LC64:
	.string	"\033[31m    Available: %d/%d chunks\n\033[0m"
	.align 8
.LC65:
	.string	"\033[36m[*] REASSEMBLING FILE: All chunks available\n\033[0m"
.LC66:
	.string	"./download"
.LC67:
	.string	"%s/%s"
.LC68:
	.string	"wb"
	.align 8
.LC69:
	.string	"\033[31m[-] ERROR: Could not create output file: %s\n\033[0m"
	.align 8
.LC70:
	.string	"\033[36m    [*] Writing chunk %d (%d bytes)\n\033[0m"
	.align 8
.LC71:
	.string	"\033[32m    DOWNLOAD SUCCESSFUL: %s\n\033[0m"
	.align 8
.LC72:
	.string	"\033[32m    Total file size: %d bytes\n\033[0m"
	.align 8
.LC73:
	.string	"\033[32m    All %d chunks retrieved and reassembled\n\033[0m"
	.text
	.globl	get_file
	.type	get_file, @function
get_file:
.LFB327:
	.loc 1 336 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	pushq	%rbx
	leaq	-28672(%rsp), %r11
.LPSRL1:
	subq	$4096, %rsp
	orq	$0, (%rsp)
	cmpq	%r11, %rsp
	jne	.LPSRL1
	subq	$2296, %rsp
	.cfi_offset 3, -24
	movq	%rdi, -30968(%rbp)
	.loc 1 336 1
	movq	%fs:40, %rax
	movq	%rax, -24(%rbp)
	xorl	%eax, %eax
	.loc 1 340 47
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 340 38
	movq	%rax, %rdi
	call	strlen@PLT
	.loc 1 340 16 discriminator 1
	movl	%eax, %edx
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	movl	%edx, %esi
	movq	%rax, %rdi
	call	str2md5
	.loc 1 340 64 discriminator 2
	movq	-30968(%rbp), %rdx
	movl	56(%rdx), %edx
	movl	%edx, %ecx
	.loc 1 340 60 discriminator 2
	movl	$0, %edx
	divl	%ecx
	movl	%edx, %eax
	.loc 1 340 9 discriminator 2
	movl	%eax, -30892(%rbp)
	.loc 1 342 9
	movl	$0, -30956(%rbp)
	.loc 1 342 16
	movl	$0, -30888(%rbp)
	.loc 1 343 22
	movq	-30968(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30856(%rbp)
	.loc 1 344 36
	movq	-30968(%rbp), %rax
	movl	56(%rax), %eax
	cltq
	.loc 1 344 27
	salq	$2, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 344 25 discriminator 1
	movq	-30968(%rbp), %rax
	movq	%rdx, 48(%rax)
	.loc 1 351 9
	movl	$0, -30952(%rbp)
	.loc 1 354 5
	leaq	-30784(%rbp), %rax
	movl	$32, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 355 5
	leaq	-30816(%rbp), %rax
	movl	$16, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 356 5
	leaq	-30800(%rbp), %rax
	movl	$16, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 359 9
	movl	$0, -30948(%rbp)
	.loc 1 360 9
	movl	$0, -30944(%rbp)
	.loc 1 364 11
	jmp	.L66
.L98:
.LBB15:
	.loc 1 366 26
	addl	$1, -30948(%rbp)
	.loc 1 368 13
	movl	-30956(%rbp), %edx
	movq	-30856(%rbp), %rcx
	movq	-30968(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	connect_server
	.loc 1 368 12 discriminator 1
	testl	%eax, %eax
	js	.L123
	.loc 1 372 26
	addl	$1, -30944(%rbp)
	.loc 1 373 9
	movl	-30956(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC39(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 376 26
	movl	-30956(%rbp), %eax
	subl	-30892(%rbp), %eax
	.loc 1 376 63
	testl	%eax, %eax
	jns	.L69
	.loc 1 376 63 is_stmt 0 discriminator 1
	movq	-30968(%rbp), %rax
	movl	56(%rax), %eax
	jmp	.L70
.L69:
	.loc 1 376 63 discriminator 2
	movl	-30956(%rbp), %eax
	subl	-30892(%rbp), %eax
.L70:
	.loc 1 376 13 is_stmt 1 discriminator 4
	movl	%eax, -30940(%rbp)
	.loc 1 378 9
	movl	-30956(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC40(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBB16:
	.loc 1 379 18
	movl	$0, -30936(%rbp)
	.loc 1 379 9
	jmp	.L71
.L72:
	.loc 1 380 34
	movl	-30940(%rbp), %edx
	movl	-30936(%rbp), %eax
	leal	(%rdx,%rax), %ecx
	.loc 1 380 43
	movq	-30968(%rbp), %rax
	movl	56(%rax), %ebx
	.loc 1 380 39
	movl	%ecx, %eax
	cltd
	idivl	%ebx
	movl	%edx, %ecx
	movl	%ecx, %eax
	.loc 1 380 13
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC41(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 379 63 discriminator 3
	addl	$1, -30936(%rbp)
.L71:
	.loc 1 379 27 discriminator 1
	cmpl	$1, -30936(%rbp)
	jle	.L72
.LBE16:
	.loc 1 382 9
	movl	$10, %edi
	call	putchar@PLT
.LBB17:
	.loc 1 385 18
	movl	$0, -30932(%rbp)
	.loc 1 385 9
	jmp	.L73
.L96:
.LBB18:
	.loc 1 387 28
	movl	-30940(%rbp), %edx
	movl	-30932(%rbp), %eax
	addl	%eax, %edx
	.loc 1 387 37
	movq	-30968(%rbp), %rax
	movl	56(%rax), %ecx
	.loc 1 387 19
	movl	%edx, %eax
	cltd
	idivl	%ecx
	movl	%edx, -30940(%rbp)
	.loc 1 390 30
	movl	-30940(%rbp), %eax
	cltq
	movl	-30816(%rbp,%rax,4), %eax
	.loc 1 390 16
	testl	%eax, %eax
	jle	.L74
	.loc 1 391 17
	movl	-30940(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC42(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 392 17
	jmp	.L95
.L74:
	.loc 1 395 13
	movl	-30956(%rbp), %eax
	leal	1(%rax), %edx
	movl	-30940(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC43(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 398 30
	movb	$0, -30832(%rbp)
	movl	-30940(%rbp), %eax
	movb	%al, -30831(%rbp)
	.loc 1 401 45
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 401 36
	movq	%rax, %rdi
	call	strlen@PLT
	.loc 1 398 30
	movl	%eax, -30828(%rbp)
	movl	$0, -30824(%rbp)
.LBB19:
	.loc 1 406 24
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30832(%rbp), %rsi
	movl	$0, %ecx
	movl	$12, %edx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 406 24 is_stmt 0 discriminator 1
	movl	%eax, -30884(%rbp)
	cmpl	$0, -30884(%rbp)
	jg	.L76
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 406 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 406 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 406 24
	jmp	.L68
.L76:
	.loc 1 406 24 discriminator 2
	movl	-30884(%rbp), %eax
.LBE19:
	.loc 1 406 22 is_stmt 1 discriminator 2
	movl	%eax, -30888(%rbp)
.LBB20:
	.loc 1 407 24
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rdi
	call	strlen@PLT
	movq	%rax, %rdx
	.loc 1 407 24 is_stmt 0 discriminator 1
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rsi
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movl	$0, %ecx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 407 24 discriminator 2
	movl	%eax, -30880(%rbp)
	cmpl	$0, -30880(%rbp)
	jg	.L78
	.loc 1 407 24 discriminator 1
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 407 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 407 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 407 24
	jmp	.L68
.L78:
	.loc 1 407 24 discriminator 2
	movl	-30880(%rbp), %eax
.LBE20:
	.loc 1 407 22 is_stmt 1 discriminator 2
	movl	%eax, -30888(%rbp)
.LBB21:
	.loc 1 410 24
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30752(%rbp), %rsi
	movl	$0, %ecx
	movl	$30720, %edx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 410 24 is_stmt 0 discriminator 1
	movl	%eax, -30876(%rbp)
	cmpl	$0, -30876(%rbp)
	jns	.L79
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 410 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 410 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC15(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 410 24
	jmp	.L68
.L79:
	.loc 1 410 24 discriminator 2
	cmpl	$0, -30876(%rbp)
	jne	.L80
	.loc 1 410 24 discriminator 3
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 410 24 discriminator 1
	leaq	.LC16(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 410 24
	jmp	.L68
.L80:
	.loc 1 410 24 discriminator 4
	movl	-30876(%rbp), %eax
.LBE21:
	.loc 1 410 22 is_stmt 1 discriminator 4
	movl	%eax, -30888(%rbp)
	.loc 1 412 17
	leaq	-30752(%rbp), %rax
	movl	$7, %edx
	leaq	.LC17(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 412 16 discriminator 1
	testl	%eax, %eax
	je	.L81
	.loc 1 414 17
	movl	-30940(%rbp), %eax
	leal	1(%rax), %edx
	movl	-30956(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC44(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 415 17
	jmp	.L95
.L81:
	.loc 1 418 13
	movl	-30940(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC45(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 421 53
	movl	$12, %edi
	call	malloc@PLT
	movq	%rax, -30848(%rbp)
.LBB22:
	.loc 1 422 24
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movq	-30848(%rbp), %rsi
	movl	$0, %ecx
	movl	$12, %edx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 422 24 is_stmt 0 discriminator 1
	movl	%eax, -30872(%rbp)
	cmpl	$0, -30872(%rbp)
	jns	.L82
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 422 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 422 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC15(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 422 24
	jmp	.L83
.L82:
	.loc 1 422 24 discriminator 2
	cmpl	$0, -30872(%rbp)
	jne	.L84
	.loc 1 422 24 discriminator 3
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 422 24 discriminator 1
	leaq	.LC16(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 422 24
	jmp	.L83
.L84:
	.loc 1 422 24 discriminator 4
	movl	-30872(%rbp), %eax
.LBE22:
	.loc 1 422 22 is_stmt 1 discriminator 4
	movl	%eax, -30888(%rbp)
	.loc 1 424 48
	movq	-30848(%rbp), %rax
	movl	8(%rax), %eax
	.loc 1 424 17
	movl	%eax, -30868(%rbp)
	.loc 1 425 86
	movq	-30848(%rbp), %rax
	movzbl	1(%rax), %eax
	movzbl	%al, %eax
	.loc 1 425 13
	leal	1(%rax), %ecx
	movl	-30868(%rbp), %eax
	movl	%eax, %edx
	movl	%ecx, %esi
	leaq	.LC46(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 428 29
	movl	-30868(%rbp), %eax
	cltq
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 428 27 discriminator 1
	movl	-30940(%rbp), %eax
	cltq
	movq	%rdx, -30784(%rbp,%rax,8)
	.loc 1 429 23
	movl	-30940(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 429 16
	testq	%rax, %rax
	jne	.L85
	.loc 1 430 17
	movl	-30940(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC47(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 431 17
	movq	-30848(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
	jmp	.L65
.L85:
	.loc 1 436 17
	movl	$0, -30928(%rbp)
	.loc 1 437 19
	jmp	.L87
.L91:
	.loc 1 439 17
	leaq	-30752(%rbp), %rax
	movl	$30720, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
.LBB23:
	.loc 1 440 28
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30752(%rbp), %rsi
	movl	$0, %ecx
	movl	$30720, %edx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 440 28 is_stmt 0 discriminator 1
	movl	%eax, -30860(%rbp)
	cmpl	$0, -30860(%rbp)
	jns	.L88
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 440 28 discriminator 2
	call	__errno_location@PLT
	.loc 1 440 28 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC15(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 440 28
	jmp	.L83
.L88:
	.loc 1 440 28 discriminator 2
	cmpl	$0, -30860(%rbp)
	jne	.L89
	.loc 1 440 28 discriminator 3
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 440 28 discriminator 1
	leaq	.LC16(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 440 28
	jmp	.L83
.L89:
	.loc 1 440 28 discriminator 4
	movl	-30860(%rbp), %eax
.LBE23:
	.loc 1 440 26 is_stmt 1 discriminator 4
	movl	%eax, -30888(%rbp)
	.loc 1 441 17
	movl	-30888(%rbp), %eax
	movslq	%eax, %rdx
	.loc 1 441 31
	movl	-30940(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rcx
	.loc 1 441 17
	movl	-30928(%rbp), %eax
	cltq
	.loc 1 441 24
	addq	%rax, %rcx
	.loc 1 441 17
	leaq	-30752(%rbp), %rax
	movq	%rax, %rsi
	movq	%rcx, %rdi
	call	memcpy@PLT
	.loc 1 442 29
	movl	-30888(%rbp), %eax
	addl	%eax, -30928(%rbp)
	.loc 1 445 20
	cmpl	$10000, -30868(%rbp)
	jle	.L87
	.loc 1 445 54 discriminator 1
	movl	-30928(%rbp), %edx
	movslq	%edx, %rax
	imulq	$1759218605, %rax, %rax
	shrq	$32, %rax
	sarl	$12, %eax
	movl	%edx, %ecx
	sarl	$31, %ecx
	subl	%ecx, %eax
	imull	$10000, %eax, %ecx
	movl	%edx, %eax
	subl	%ecx, %eax
	.loc 1 445 39 discriminator 1
	testl	%eax, %eax
	jne	.L87
	.loc 1 447 52
	pxor	%xmm0, %xmm0
	cvtsi2ssl	-30928(%rbp), %xmm0
	.loc 1 447 70
	pxor	%xmm2, %xmm2
	cvtsi2ssl	-30868(%rbp), %xmm2
	movaps	%xmm0, %xmm1
	divss	%xmm2, %xmm1
	.loc 1 447 80
	movss	.LC48(%rip), %xmm0
	mulss	%xmm1, %xmm0
	.loc 1 446 21
	pxor	%xmm3, %xmm3
	cvtss2sd	%xmm0, %xmm3
	movq	%xmm3, %rcx
	movl	-30868(%rbp), %edx
	movl	-30928(%rbp), %eax
	movq	%rcx, %xmm0
	movl	%eax, %esi
	leaq	.LC49(%rip), %rax
	movq	%rax, %rdi
	movl	$1, %eax
	call	printf@PLT
.L87:
	.loc 1 437 32
	movl	-30928(%rbp), %eax
	cmpl	-30868(%rbp), %eax
	jl	.L91
.LBB24:
	.loc 1 452 24
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movl	$0, %ecx
	movl	$7, %edx
	leaq	.LC17(%rip), %rsi
	movl	%eax, %edi
	call	send@PLT
	.loc 1 452 24 is_stmt 0 discriminator 1
	movl	%eax, -30864(%rbp)
	cmpl	$0, -30864(%rbp)
	jg	.L92
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 452 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 452 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 452 24
	jmp	.L83
.L92:
	.loc 1 452 24 discriminator 2
	movl	-30864(%rbp), %eax
.LBE24:
	.loc 1 452 22 is_stmt 1 discriminator 2
	movl	%eax, -30888(%rbp)
	.loc 1 455 34
	movl	-30940(%rbp), %eax
	cltq
	movl	$1, -30816(%rbp,%rax,4)
	.loc 1 456 40
	movl	-30940(%rbp), %eax
	cltq
	movl	-30868(%rbp), %edx
	movl	%edx, -30800(%rbp,%rax,4)
	.loc 1 457 36
	addl	$1, -30952(%rbp)
	.loc 1 459 13
	movl	-30940(%rbp), %eax
	leal	1(%rax), %ecx
	movl	-30868(%rbp), %eax
	movl	%eax, %edx
	movl	%ecx, %esi
	leaq	.LC50(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 461 13
	movq	-30848(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 462 13
	jmp	.L95
.L83:
	.loc 1 465 13
	movl	-30940(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC51(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 466 16
	cmpq	$0, -30848(%rbp)
	je	.L93
	.loc 1 466 38 discriminator 1
	movq	-30848(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
.L93:
	.loc 1 467 23
	movl	-30940(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 467 16
	testq	%rax, %rax
	je	.L95
	.loc 1 468 28
	movl	-30940(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 468 17
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 469 31
	movl	-30940(%rbp), %eax
	cltq
	movq	$0, -30784(%rbp,%rax,8)
.L95:
.LBE18:
	.loc 1 385 63 discriminator 2
	addl	$1, -30932(%rbp)
.L73:
	.loc 1 385 27 discriminator 1
	cmpl	$1, -30932(%rbp)
	jle	.L96
.LBE17:
	.loc 1 473 5
	jmp	.L68
.L123:
	.loc 1 369 13
	nop
.L68:
	.loc 1 474 9
	movq	-30856(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 475 17
	movq	-30856(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30856(%rbp)
	.loc 1 476 10
	addl	$1, -30956(%rbp)
.L66:
.LBE15:
	.loc 1 364 20
	cmpq	$0, -30856(%rbp)
	je	.L97
	.loc 1 364 20 is_stmt 0 discriminator 1
	cmpl	$3, -30952(%rbp)
	jle	.L98
.L97:
	.loc 1 480 5 is_stmt 1
	leaq	.LC52(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 481 5
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC53(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 482 5
	leaq	.LC54(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 484 5
	movl	-30948(%rbp), %edx
	movl	-30944(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC55(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 485 5
	movl	-30952(%rbp), %eax
	movl	$4, %edx
	movl	%eax, %esi
	leaq	.LC56(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 488 5
	leaq	.LC57(%rip), %rax
	movq	%rax, %rdi
	call	puts@PLT
.LBB25:
	.loc 1 489 14
	movl	$0, -30924(%rbp)
	.loc 1 489 5
	jmp	.L99
.L102:
	.loc 1 491 26
	movl	-30924(%rbp), %eax
	cltq
	movl	-30816(%rbp,%rax,4), %eax
	.loc 1 491 12
	testl	%eax, %eax
	jle	.L100
	.loc 1 492 13
	movl	-30924(%rbp), %eax
	cltq
	movl	-30800(%rbp,%rax,4), %eax
	movl	-30924(%rbp), %edx
	leal	1(%rdx), %ecx
	movl	%eax, %edx
	movl	%ecx, %esi
	leaq	.LC58(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L101
.L100:
	.loc 1 494 13
	movl	-30924(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC59(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L101:
	.loc 1 489 43 discriminator 2
	addl	$1, -30924(%rbp)
.L99:
	.loc 1 489 23 discriminator 1
	cmpl	$3, -30924(%rbp)
	jle	.L102
.LBE25:
	.loc 1 497 5
	movl	$10, %edi
	call	putchar@PLT
	.loc 1 500 9
	movl	$1, -30920(%rbp)
.LBB26:
	.loc 1 501 14
	movl	$0, -30916(%rbp)
	.loc 1 501 5
	jmp	.L103
.L106:
	.loc 1 503 19
	movl	-30916(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 503 12
	testq	%rax, %rax
	je	.L104
	.loc 1 503 53 discriminator 1
	movl	-30916(%rbp), %eax
	cltq
	movl	-30800(%rbp,%rax,4), %eax
	.loc 1 503 31 discriminator 1
	testl	%eax, %eax
	jle	.L104
	.loc 1 503 78 discriminator 2
	movl	-30916(%rbp), %eax
	cltq
	movl	-30816(%rbp,%rax,4), %eax
	.loc 1 503 62 discriminator 2
	testl	%eax, %eax
	jg	.L105
.L104:
	.loc 1 505 25
	movl	$0, -30920(%rbp)
	.loc 1 506 13
	movl	-30916(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC60(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L105:
	.loc 1 501 43 discriminator 2
	addl	$1, -30916(%rbp)
.L103:
	.loc 1 501 23 discriminator 1
	cmpl	$3, -30916(%rbp)
	jle	.L106
.LBE26:
	.loc 1 510 8
	cmpl	$0, -30920(%rbp)
	jne	.L107
	.loc 1 512 9
	leaq	.LC20(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 513 9
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC61(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 514 9
	leaq	.LC62(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 515 9
	movl	$4, %edx
	movl	$4, %esi
	leaq	.LC63(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 516 9
	movl	-30952(%rbp), %eax
	movl	$4, %edx
	movl	%eax, %esi
	leaq	.LC64(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 517 9
	leaq	.LC25(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBB27:
	.loc 1 520 18
	movl	$0, -30912(%rbp)
	.loc 1 520 9
	jmp	.L108
.L110:
	.loc 1 521 23
	movl	-30912(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 521 16
	testq	%rax, %rax
	je	.L109
	.loc 1 521 47 discriminator 1
	movl	-30912(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 521 36 discriminator 1
	movq	%rax, %rdi
	call	free@PLT
.L109:
	.loc 1 520 47 discriminator 2
	addl	$1, -30912(%rbp)
.L108:
	.loc 1 520 27 discriminator 1
	cmpl	$3, -30912(%rbp)
	jle	.L110
.LBE27:
	.loc 1 524 16
	movq	-30968(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 524 9
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 525 9
	jmp	.L65
.L107:
	.loc 1 529 5
	leaq	.LC65(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 532 11
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 532 8
	movzbl	(%rax), %eax
	.loc 1 532 7
	cmpb	$46, %al
	jne	.L111
	.loc 1 533 64
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 533 9
	leaq	2(%rax), %rdx
	leaq	-30832(%rbp), %rax
	movq	%rdx, %rcx
	leaq	.LC66(%rip), %rdx
	leaq	.LC67(%rip), %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	asprintf@PLT
	jmp	.L112
.L111:
	.loc 1 535 9
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rdx
	leaq	-30832(%rbp), %rax
	movq	%rdx, %rcx
	leaq	.LC66(%rip), %rdx
	leaq	.LC67(%rip), %rsi
	movq	%rax, %rdi
	movl	$0, %eax
	call	asprintf@PLT
.L112:
	.loc 1 537 16
	movq	-30832(%rbp), %rax
	leaq	.LC68(%rip), %rdx
	movq	%rdx, %rsi
	movq	%rax, %rdi
	call	fopen@PLT
	movq	%rax, -30840(%rbp)
	.loc 1 538 8
	cmpq	$0, -30840(%rbp)
	jne	.L113
	.loc 1 539 9
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC69(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBB28:
	.loc 1 542 18
	movl	$0, -30908(%rbp)
	.loc 1 542 9
	jmp	.L114
.L116:
	.loc 1 543 23
	movl	-30908(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 543 16
	testq	%rax, %rax
	je	.L115
	.loc 1 543 47 discriminator 1
	movl	-30908(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 543 36 discriminator 1
	movq	%rax, %rdi
	call	free@PLT
.L115:
	.loc 1 542 47 discriminator 2
	addl	$1, -30908(%rbp)
.L114:
	.loc 1 542 27 discriminator 1
	cmpl	$3, -30908(%rbp)
	jle	.L116
.LBE28:
	.loc 1 546 16
	movq	-30968(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 546 9
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 547 9
	jmp	.L65
.L113:
	.loc 1 549 5
	movq	-30832(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 552 9
	movl	$0, -30904(%rbp)
.LBB29:
	.loc 1 553 14
	movl	$0, -30900(%rbp)
	.loc 1 553 5
	jmp	.L117
.L118:
	.loc 1 554 47
	movl	-30900(%rbp), %eax
	cltq
	movl	-30800(%rbp,%rax,4), %eax
	.loc 1 554 25
	addl	%eax, -30904(%rbp)
	.loc 1 553 43 discriminator 3
	addl	$1, -30900(%rbp)
.L117:
	.loc 1 553 23 discriminator 1
	cmpl	$3, -30900(%rbp)
	jle	.L118
.LBE29:
.LBB30:
	.loc 1 558 14
	movl	$0, -30896(%rbp)
	.loc 1 558 5
	jmp	.L119
.L120:
	.loc 1 560 9
	movl	-30896(%rbp), %eax
	cltq
	movl	-30800(%rbp,%rax,4), %eax
	movl	-30896(%rbp), %edx
	leal	1(%rdx), %ecx
	movl	%eax, %edx
	movl	%ecx, %esi
	leaq	.LC70(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 561 46
	movl	-30896(%rbp), %eax
	cltq
	movl	-30800(%rbp,%rax,4), %eax
	.loc 1 561 9
	movslq	%eax, %rsi
	.loc 1 561 22
	movl	-30896(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 561 9
	movq	-30840(%rbp), %rdx
	movq	%rdx, %rcx
	movl	$1, %edx
	movq	%rax, %rdi
	call	fwrite@PLT
	.loc 1 562 20
	movl	-30896(%rbp), %eax
	cltq
	movq	-30784(%rbp,%rax,8), %rax
	.loc 1 562 9
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 558 43 discriminator 3
	addl	$1, -30896(%rbp)
.L119:
	.loc 1 558 23 discriminator 1
	cmpl	$3, -30896(%rbp)
	jle	.L120
.LBE30:
	.loc 1 564 5
	movq	-30840(%rbp), %rax
	movq	%rax, %rdi
	call	fclose@PLT
	.loc 1 566 5
	leaq	.LC26(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 567 5
	movq	-30968(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC71(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 568 5
	movl	-30904(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC72(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 569 5
	movl	$4, %esi
	leaq	.LC73(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 570 5
	leaq	.LC30(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 572 12
	movq	-30968(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 572 5
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 573 25
	movq	-30968(%rbp), %rax
	movq	$0, 48(%rax)
.L65:
	.loc 1 574 1
	movq	-24(%rbp), %rax
	subq	%fs:40, %rax
	je	.L122
	call	__stack_chk_fail@PLT
.L122:
	movq	-8(%rbp), %rbx
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE327:
	.size	get_file, .-get_file
	.globl	extract_original_filename
	.type	extract_original_filename, @function
extract_original_filename:
.LFB328:
	.loc 1 579 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$48, %rsp
	movq	%rdi, -40(%rbp)
	.loc 1 581 36
	movq	-40(%rbp), %rax
	movl	$95, %esi
	movq	%rax, %rdi
	call	strchr@PLT
	movq	%rax, -32(%rbp)
	.loc 1 582 8
	cmpq	$0, -32(%rbp)
	jne	.L125
	.loc 1 584 16
	movl	$0, %eax
	jmp	.L126
.L125:
	.loc 1 588 21
	addq	$1, -32(%rbp)
	.loc 1 591 35
	movq	-32(%rbp), %rax
	movl	$95, %esi
	movq	%rax, %rdi
	call	strrchr@PLT
	movq	%rax, -24(%rbp)
	.loc 1 592 8
	cmpq	$0, -24(%rbp)
	jne	.L127
	.loc 1 594 16
	movl	$0, %eax
	jmp	.L126
.L127:
	.loc 1 598 43
	movq	-24(%rbp), %rax
	subq	-32(%rbp), %rax
	.loc 1 598 12
	movq	%rax, -16(%rbp)
	.loc 1 601 31
	movq	-16(%rbp), %rax
	addq	$1, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, -8(%rbp)
	.loc 1 602 8
	cmpq	$0, -8(%rbp)
	jne	.L128
	.loc 1 604 16
	movl	$0, %eax
	jmp	.L126
.L128:
	.loc 1 608 5
	movq	-16(%rbp), %rdx
	movq	-32(%rbp), %rcx
	movq	-8(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncpy@PLT
	.loc 1 609 22
	movq	-8(%rbp), %rdx
	movq	-16(%rbp), %rax
	addq	%rdx, %rax
	.loc 1 609 37
	movb	$0, (%rax)
	.loc 1 611 12
	movq	-8(%rbp), %rax
.L126:
	.loc 1 612 1
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE328:
	.size	extract_original_filename, .-extract_original_filename
	.globl	parse_server_filename
	.type	parse_server_filename, @function
parse_server_filename:
.LFB329:
	.loc 1 616 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$64, %rsp
	movq	%rdi, -40(%rbp)
	movq	%rsi, -48(%rbp)
	movq	%rdx, -56(%rbp)
	.loc 1 618 33
	movq	-40(%rbp), %rax
	movl	$47, %esi
	movq	%rax, %rdi
	call	strrchr@PLT
	movq	%rax, -24(%rbp)
	.loc 1 619 8
	cmpq	$0, -24(%rbp)
	je	.L130
	.loc 1 621 22
	addq	$1, -24(%rbp)
	jmp	.L131
.L130:
	.loc 1 625 23
	movq	-40(%rbp), %rax
	movq	%rax, -24(%rbp)
.L131:
	.loc 1 629 35
	movq	-24(%rbp), %rax
	movl	$95, %esi
	movq	%rax, %rdi
	call	strrchr@PLT
	movq	%rax, -16(%rbp)
	.loc 1 630 8
	cmpq	$0, -16(%rbp)
	jne	.L132
	.loc 1 632 16
	movl	$-1, %eax
	jmp	.L133
.L132:
	.loc 1 636 17
	movq	-16(%rbp), %rax
	addq	$1, %rax
	movq	%rax, %rdi
	call	atoi@PLT
	.loc 1 636 15 discriminator 1
	movq	-56(%rbp), %rdx
	movl	%eax, (%rdx)
	.loc 1 639 43
	movq	-16(%rbp), %rax
	subq	-24(%rbp), %rax
	.loc 1 639 12
	movq	%rax, -8(%rbp)
	.loc 1 642 26
	movq	-8(%rbp), %rax
	addq	$1, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 642 24 discriminator 1
	movq	-48(%rbp), %rax
	movq	%rdx, (%rax)
	.loc 1 643 9
	movq	-48(%rbp), %rax
	movq	(%rax), %rax
	.loc 1 643 8
	testq	%rax, %rax
	jne	.L134
	.loc 1 645 16
	movl	$-1, %eax
	jmp	.L133
.L134:
	.loc 1 649 5
	movq	-48(%rbp), %rax
	movq	(%rax), %rax
	movq	-8(%rbp), %rdx
	movq	-24(%rbp), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncpy@PLT
	.loc 1 650 6
	movq	-48(%rbp), %rax
	movq	(%rax), %rdx
	.loc 1 650 25
	movq	-8(%rbp), %rax
	addq	%rdx, %rax
	.loc 1 650 40
	movb	$0, (%rax)
	.loc 1 652 12
	movl	$0, %eax
.L133:
	.loc 1 653 1
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE329:
	.size	parse_server_filename, .-parse_server_filename
	.section	.rodata
	.align 8
.LC74:
	.string	"\033[36m\n[*] SCANNING SERVERS FOR FILES\n\033[0m"
	.align 8
.LC75:
	.string	"\033[36m\n[*] QUERYING: Server %d (%s:%s)\n\033[0m"
	.align 8
.LC76:
	.string	"\033[36m[*] LIST REQUEST SENT\n\033[0m"
.LC77:
	.string	"nack\t\n0\t\n"
	.string	""
	.align 8
.LC78:
	.string	"\033[36m    [*] Chunk %d of %s found on server %d\n\033[0m"
	.align 8
.LC79:
	.string	"\033[31m    [-] ERROR: Could not parse filename: %s\n\033[0m"
	.align 8
.LC80:
	.string	"\033[33m    [!] No files found on server %d\n\033[0m"
	.align 8
.LC81:
	.string	"\033[32m    [+] Found %d file chunks on server %d\n\033[0m"
	.align 8
.LC82:
	.string	"\033[36m    FILE AVAILABILITY SUMMARY\n\033[0m"
	.align 8
.LC83:
	.string	"\033[33m[!] No files found on any server\n\n\033[0m"
.LC84:
	.string	"CHUNKS AVAILABLE"
.LC85:
	.string	"STATUS"
.LC86:
	.string	"FILENAME"
.LC87:
	.string	"%-30s %-15s %-15s\n"
	.align 8
.LC88:
	.string	"--------------------------------------------------------------"
.LC89:
	.string	"%-30s "
.LC90:
	.string	"COMPLETE"
.LC91:
	.string	"\033[32m%-15s\033[0m"
.LC92:
	.string	"INCOMPLETE"
.LC93:
	.string	"\033[31m%-15s\033[0m"
.LC94:
	.string	"%-2d/%-2d"
.LC95:
	.string	" ["
.LC96:
	.string	"\033[32m%d\033[0m"
.LC97:
	.string	"\033[31mX\033[0m"
.LC98:
	.string	"]"
	.text
	.globl	list_file
	.type	list_file, @function
list_file:
.LFB330:
	.loc 1 656 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	leaq	-28672(%rsp), %r11
.LPSRL2:
	subq	$4096, %rsp
	orq	$0, (%rsp)
	cmpq	%r11, %rsp
	jne	.LPSRL2
	subq	$2192, %rsp
	movq	%rdi, -30856(%rbp)
	.loc 1 656 1
	movq	%fs:40, %rax
	movq	%rax, -8(%rbp)
	xorl	%eax, %eax
	.loc 1 657 5
	leaq	.LC74(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 659 9
	movl	$0, -30832(%rbp)
	.loc 1 659 16
	movl	$0, -30804(%rbp)
	.loc 1 660 22
	movq	-30856(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30792(%rbp)
	.loc 1 661 36
	movq	-30856(%rbp), %rax
	movl	56(%rax), %eax
	cltq
	.loc 1 661 27
	salq	$2, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 661 25 discriminator 1
	movq	-30856(%rbp), %rax
	movq	%rdx, 48(%rax)
	.loc 1 671 18
	movq	$0, -30784(%rbp)
	.loc 1 674 11
	jmp	.L136
.L151:
.LBB31:
	.loc 1 676 9
	movq	-30792(%rbp), %rax
	movq	8(%rax), %rdx
	movq	-30792(%rbp), %rax
	movq	16(%rax), %rax
	movl	-30832(%rbp), %ecx
	leal	1(%rcx), %esi
	movq	%rdx, %rcx
	movq	%rax, %rdx
	leaq	.LC75(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 678 13
	movl	-30832(%rbp), %edx
	movq	-30792(%rbp), %rcx
	movq	-30856(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	connect_server
	.loc 1 678 12 discriminator 1
	testl	%eax, %eax
	js	.L168
	.loc 1 682 26
	movb	$3, -30748(%rbp)
	movb	$0, -30747(%rbp)
	movl	$0, -30744(%rbp)
	movl	$0, -30740(%rbp)
	.loc 1 688 20
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30748(%rbp), %rsi
	movl	$0, %ecx
	movl	$12, %edx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 688 18 discriminator 1
	movl	%eax, -30804(%rbp)
	.loc 1 689 9
	leaq	.LC76(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 691 13
	movl	$0, -30828(%rbp)
.L148:
.LBB32:
	.loc 1 696 13
	leaq	-30748(%rbp), %rax
	movl	$12, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 697 24
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30748(%rbp), %rsi
	movl	$0, %ecx
	movl	$12, %edx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 697 22 discriminator 1
	movl	%eax, -30804(%rbp)
	.loc 1 698 16
	cmpl	$0, -30804(%rbp)
	jle	.L169
	.loc 1 701 13
	movl	-30804(%rbp), %eax
	movslq	%eax, %rdx
	leaq	-30748(%rbp), %rcx
	leaq	-30736(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	memcpy@PLT
	.loc 1 702 17
	leaq	-30736(%rbp), %rax
	movl	$7, %edx
	leaq	.LC17(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 702 16 discriminator 1
	testl	%eax, %eax
	je	.L140
	.loc 1 702 57 discriminator 1
	leaq	-30736(%rbp), %rax
	movl	$8, %edx
	leaq	.LC77(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 702 54 discriminator 1
	testl	%eax, %eax
	je	.L140
	.loc 1 705 13
	leaq	-30736(%rbp), %rax
	movl	$30720, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 706 84
	movl	-30744(%rbp), %eax
	.loc 1 706 24
	movl	%eax, %edx
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30736(%rbp), %rsi
	movl	$0, %ecx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 706 22 discriminator 1
	movl	%eax, -30804(%rbp)
	.loc 1 707 16
	cmpl	$0, -30804(%rbp)
	jle	.L140
	.loc 1 707 34 discriminator 1
	leaq	-30736(%rbp), %rax
	movl	$7, %edx
	leaq	.LC17(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 707 31 discriminator 1
	testl	%eax, %eax
	je	.L140
	.loc 1 707 74 discriminator 2
	leaq	-30736(%rbp), %rax
	movl	$8, %edx
	leaq	.LC77(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 707 71 discriminator 1
	testl	%eax, %eax
	je	.L140
	.loc 1 711 19
	movq	$0, -30800(%rbp)
	.loc 1 712 17
	movl	$-1, -30836(%rbp)
	.loc 1 713 17
	leaq	-30836(%rbp), %rdx
	leaq	-30800(%rbp), %rcx
	leaq	-30736(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	parse_server_filename
	.loc 1 713 16 discriminator 1
	testl	%eax, %eax
	jne	.L141
	.loc 1 713 103 discriminator 1
	movl	-30836(%rbp), %eax
	.loc 1 713 91 discriminator 1
	cmpl	$3, %eax
	jg	.L141
.LBB33:
	.loc 1 715 28
	addl	$1, -30828(%rbp)
	.loc 1 718 30
	movq	-30784(%rbp), %rax
	movq	%rax, -30776(%rbp)
	.loc 1 719 21
	movl	$0, -30824(%rbp)
	.loc 1 721 23
	jmp	.L142
.L145:
	.loc 1 723 25
	movq	-30800(%rbp), %rdx
	.loc 1 723 42
	movq	-30776(%rbp), %rax
	movq	(%rax), %rax
	.loc 1 723 25
	movq	%rdx, %rsi
	movq	%rax, %rdi
	call	strcmp@PLT
	.loc 1 723 24 discriminator 1
	testl	%eax, %eax
	jne	.L143
	.loc 1 725 31
	movl	$1, -30824(%rbp)
	.loc 1 726 51
	movl	-30836(%rbp), %edx
	movq	-30776(%rbp), %rax
	movslq	%edx, %rdx
	movl	8(%rax,%rdx,4), %eax
	movl	-30836(%rbp), %edx
	.loc 1 726 62
	leal	1(%rax), %ecx
	movq	-30776(%rbp), %rax
	movslq	%edx, %rdx
	movl	%ecx, 8(%rax,%rdx,4)
	.loc 1 727 25
	jmp	.L144
.L143:
	.loc 1 729 32
	movq	-30776(%rbp), %rax
	movq	24(%rax), %rax
	movq	%rax, -30776(%rbp)
.L142:
	.loc 1 721 35
	cmpq	$0, -30776(%rbp)
	jne	.L145
.L144:
	.loc 1 733 20
	cmpl	$0, -30824(%rbp)
	jne	.L146
	.loc 1 735 34
	movl	$32, %edi
	call	malloc@PLT
	movq	%rax, -30776(%rbp)
	.loc 1 736 44
	movq	-30800(%rbp), %rax
	movq	%rax, %rdi
	call	strdup@PLT
	movq	%rax, %rdx
	.loc 1 736 42 discriminator 1
	movq	-30776(%rbp), %rax
	movq	%rdx, (%rax)
	.loc 1 737 38
	movq	-30776(%rbp), %rax
	addq	$8, %rax
	.loc 1 737 21
	movl	$16, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
	.loc 1 738 47
	movl	-30836(%rbp), %edx
	.loc 1 738 58
	movq	-30776(%rbp), %rax
	movslq	%edx, %rdx
	movl	$1, 8(%rax,%rdx,4)
	.loc 1 739 38
	movq	-30776(%rbp), %rax
	movq	-30784(%rbp), %rdx
	movq	%rdx, 24(%rax)
	.loc 1 740 31
	movq	-30776(%rbp), %rax
	movq	%rax, -30784(%rbp)
.L146:
	.loc 1 743 17
	movl	-30832(%rbp), %eax
	leal	1(%rax), %edx
	movq	-30800(%rbp), %rax
	movl	-30836(%rbp), %ecx
	leal	1(%rcx), %esi
	movl	%edx, %ecx
	movq	%rax, %rdx
	leaq	.LC78(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 745 17
	movq	-30800(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
.LBE33:
	.loc 1 714 13
	jmp	.L147
.L141:
	.loc 1 749 17
	leaq	-30736(%rbp), %rax
	movq	%rax, %rsi
	leaq	.LC79(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L147:
.LBE32:
	.loc 1 695 9
	jmp	.L148
.L169:
.LBB34:
	.loc 1 699 17
	nop
.L140:
.LBE34:
	.loc 1 753 12
	cmpl	$0, -30828(%rbp)
	jne	.L149
	.loc 1 754 13
	movl	-30832(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC80(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L138
.L149:
	.loc 1 756 13
	movl	-30832(%rbp), %eax
	leal	1(%rax), %edx
	movl	-30828(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC81(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L138
.L168:
	.loc 1 679 13
	nop
.L138:
	.loc 1 760 20
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	.loc 1 760 12
	testl	%eax, %eax
	js	.L150
	.loc 1 761 13
	movq	-30792(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
.L150:
	.loc 1 763 17
	movq	-30792(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30792(%rbp)
	.loc 1 764 10
	addl	$1, -30832(%rbp)
.L136:
.LBE31:
	.loc 1 674 12
	cmpq	$0, -30792(%rbp)
	jne	.L151
	.loc 1 768 5
	leaq	.LC52(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 769 5
	leaq	.LC82(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 770 5
	leaq	.LC54(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 772 8
	cmpq	$0, -30784(%rbp)
	jne	.L152
	.loc 1 773 9
	leaq	.LC83(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L153
.L152:
.LBB35:
	.loc 1 776 9
	leaq	.LC84(%rip), %rax
	movq	%rax, %rcx
	leaq	.LC85(%rip), %rax
	movq	%rax, %rdx
	leaq	.LC86(%rip), %rax
	movq	%rax, %rsi
	leaq	.LC87(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 777 9
	leaq	.LC88(%rip), %rax
	movq	%rax, %rdi
	call	puts@PLT
	.loc 1 780 22
	movq	-30784(%rbp), %rax
	movq	%rax, -30768(%rbp)
	.loc 1 781 15
	jmp	.L154
.L166:
.LBB36:
	.loc 1 784 17
	movl	$0, -30820(%rbp)
	.loc 1 785 17
	movl	$1, -30816(%rbp)
.LBB37:
	.loc 1 787 22
	movl	$0, -30812(%rbp)
	.loc 1 787 13
	jmp	.L155
.L158:
	.loc 1 789 47
	movq	-30768(%rbp), %rax
	movl	-30812(%rbp), %edx
	movslq	%edx, %rdx
	movl	8(%rax,%rdx,4), %eax
	.loc 1 789 20
	testl	%eax, %eax
	jle	.L156
	.loc 1 791 37
	addl	$1, -30820(%rbp)
	jmp	.L157
.L156:
	.loc 1 795 33
	movl	$0, -30816(%rbp)
.L157:
	.loc 1 787 51 discriminator 2
	addl	$1, -30812(%rbp)
.L155:
	.loc 1 787 31 discriminator 1
	cmpl	$3, -30812(%rbp)
	jle	.L158
.LBE37:
	.loc 1 800 13
	movq	-30768(%rbp), %rax
	movq	(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC89(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 802 16
	cmpl	$0, -30816(%rbp)
	je	.L159
	.loc 1 803 17
	leaq	.LC90(%rip), %rax
	movq	%rax, %rsi
	leaq	.LC91(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L160
.L159:
	.loc 1 805 17
	leaq	.LC92(%rip), %rax
	movq	%rax, %rsi
	leaq	.LC93(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L160:
	.loc 1 808 13
	movl	-30820(%rbp), %eax
	movl	$4, %edx
	movl	%eax, %esi
	leaq	.LC94(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 811 13
	leaq	.LC95(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.LBB38:
	.loc 1 812 22
	movl	$0, -30808(%rbp)
	.loc 1 812 13
	jmp	.L161
.L165:
	.loc 1 814 47
	movq	-30768(%rbp), %rax
	movl	-30808(%rbp), %edx
	movslq	%edx, %rdx
	movl	8(%rax,%rdx,4), %eax
	.loc 1 814 20
	testl	%eax, %eax
	jle	.L162
	.loc 1 816 21
	movl	-30808(%rbp), %eax
	addl	$1, %eax
	movl	%eax, %esi
	leaq	.LC96(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L163
.L162:
	.loc 1 820 21
	leaq	.LC97(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L163:
	.loc 1 823 20
	cmpl	$2, -30808(%rbp)
	jg	.L164
	.loc 1 824 21
	movl	$44, %edi
	call	putchar@PLT
.L164:
	.loc 1 812 51 discriminator 2
	addl	$1, -30808(%rbp)
.L161:
	.loc 1 812 31 discriminator 1
	cmpl	$3, -30808(%rbp)
	jle	.L165
.LBE38:
	.loc 1 826 13
	leaq	.LC98(%rip), %rax
	movq	%rax, %rdi
	call	puts@PLT
	.loc 1 829 26
	movq	-30768(%rbp), %rax
	movq	%rax, -30760(%rbp)
	.loc 1 830 24
	movq	-30768(%rbp), %rax
	movq	24(%rax), %rax
	movq	%rax, -30768(%rbp)
	.loc 1 832 22
	movq	-30760(%rbp), %rax
	movq	(%rax), %rax
	.loc 1 832 13
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 833 13
	movq	-30760(%rbp), %rax
	movq	%rax, %rdi
	call	free@PLT
.L154:
.LBE36:
	.loc 1 781 27
	cmpq	$0, -30768(%rbp)
	jne	.L166
	.loc 1 835 9
	movl	$10, %edi
	call	putchar@PLT
.L153:
.LBE35:
	.loc 1 839 12
	movq	-30856(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 839 5
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 840 25
	movq	-30856(%rbp), %rax
	movq	$0, 48(%rax)
	.loc 1 841 1
	nop
	movq	-8(%rbp), %rax
	subq	%fs:40, %rax
	je	.L167
	call	__stack_chk_fail@PLT
.L167:
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE330:
	.size	list_file, .-list_file
	.section	.rodata
	.align 8
.LC99:
	.string	"\033[32m    FILE DELETE SUCCESSFUL\n\033[0m"
	.align 8
.LC100:
	.string	"\033[32m    All chunks removed from servers\n\033[0m"
	.align 8
.LC101:
	.string	"\033[31m[-] ERROR: File not found or permission denied on server\n\033[0m"
	.text
	.globl	delete_file
	.type	delete_file, @function
delete_file:
.LFB331:
	.loc 1 844 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	pushq	%rbx
	leaq	-28672(%rsp), %r11
.LPSRL3:
	subq	$4096, %rsp
	orq	$0, (%rsp)
	cmpq	%r11, %rsp
	jne	.LPSRL3
	subq	$2152, %rsp
	.cfi_offset 3, -24
	movq	%rdi, -30824(%rbp)
	.loc 1 844 1
	movq	%fs:40, %rax
	movq	%rax, -24(%rbp)
	xorl	%eax, %eax
	.loc 1 846 9
	movl	$0, -30808(%rbp)
	.loc 1 846 16
	movl	$0, -30796(%rbp)
	.loc 1 847 22
	movq	-30824(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30776(%rbp)
	.loc 1 848 36
	movq	-30824(%rbp), %rax
	movl	56(%rax), %eax
	cltq
	.loc 1 848 27
	salq	$2, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 848 25 discriminator 1
	movq	-30824(%rbp), %rax
	movq	%rdx, 48(%rax)
	.loc 1 851 11
	jmp	.L171
.L185:
.LBB39:
	.loc 1 853 13
	movl	-30808(%rbp), %edx
	movq	-30776(%rbp), %rcx
	movq	-30824(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	connect_server
	.loc 1 853 12 discriminator 1
	testl	%eax, %eax
	js	.L187
	.loc 1 856 51
	movq	-30824(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 856 42
	movq	%rax, %rdi
	call	strlen@PLT
	.loc 1 856 20 discriminator 1
	movl	%eax, %edx
	movq	-30824(%rbp), %rax
	movq	16(%rax), %rax
	movl	%edx, %esi
	movq	%rax, %rdi
	call	str2md5
	.loc 1 856 68 discriminator 2
	movq	-30824(%rbp), %rdx
	movl	56(%rdx), %edx
	movl	%edx, %ecx
	.loc 1 856 64 discriminator 2
	movl	$0, %edx
	divl	%ecx
	movl	%edx, %eax
	.loc 1 856 13 discriminator 2
	movl	%eax, -30792(%rbp)
	.loc 1 857 26
	movl	-30808(%rbp), %eax
	subl	-30792(%rbp), %eax
	.loc 1 857 63
	testl	%eax, %eax
	jns	.L174
	.loc 1 857 63 is_stmt 0 discriminator 1
	movq	-30824(%rbp), %rax
	movl	56(%rax), %eax
	jmp	.L175
.L174:
	.loc 1 857 63 discriminator 2
	movl	-30808(%rbp), %eax
	subl	-30792(%rbp), %eax
.L175:
	.loc 1 857 13 is_stmt 1 discriminator 4
	movl	%eax, -30804(%rbp)
.LBB40:
	.loc 1 858 18
	movl	$0, -30800(%rbp)
	.loc 1 858 9
	jmp	.L176
.L184:
.LBB41:
	.loc 1 860 28
	movl	-30804(%rbp), %edx
	movl	-30800(%rbp), %eax
	addl	%eax, %edx
	.loc 1 860 37
	movq	-30824(%rbp), %rax
	movl	56(%rax), %ecx
	.loc 1 860 19
	movl	%edx, %eax
	cltd
	idivl	%ecx
	movl	%edx, -30804(%rbp)
	.loc 1 862 30
	movb	$2, -30764(%rbp)
	movl	-30804(%rbp), %eax
	movb	%al, -30763(%rbp)
	.loc 1 865 45
	movq	-30824(%rbp), %rax
	movq	16(%rax), %rax
	.loc 1 865 36
	movq	%rax, %rdi
	call	strlen@PLT
	.loc 1 862 30
	movl	%eax, -30760(%rbp)
	movl	$0, -30756(%rbp)
.LBB42:
	.loc 1 868 24
	movq	-30776(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30764(%rbp), %rsi
	movl	$0, %ecx
	movl	$12, %edx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 868 24 is_stmt 0 discriminator 1
	movl	%eax, -30788(%rbp)
	cmpl	$0, -30788(%rbp)
	jg	.L177
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 868 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 868 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 868 24
	jmp	.L173
.L177:
	.loc 1 868 24 discriminator 2
	movl	-30788(%rbp), %eax
.LBE42:
	.loc 1 868 22 is_stmt 1 discriminator 2
	movl	%eax, -30796(%rbp)
.LBB43:
	.loc 1 869 24
	movl	-30760(%rbp), %eax
	movl	%eax, %edx
	movq	-30824(%rbp), %rax
	movq	16(%rax), %rsi
	movq	-30776(%rbp), %rax
	movl	(%rax), %eax
	movl	$0, %ecx
	movl	%eax, %edi
	call	send@PLT
	.loc 1 869 24 is_stmt 0 discriminator 1
	movl	%eax, -30784(%rbp)
	cmpl	$0, -30784(%rbp)
	jg	.L179
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 869 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 869 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC14(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 869 24
	jmp	.L173
.L179:
	.loc 1 869 24 discriminator 2
	movl	-30784(%rbp), %eax
.LBE43:
	.loc 1 869 22 is_stmt 1 discriminator 2
	movl	%eax, -30796(%rbp)
	.loc 1 870 13
	leaq	-30752(%rbp), %rax
	movl	$30720, %edx
	movl	$0, %esi
	movq	%rax, %rdi
	call	memset@PLT
.LBB44:
	.loc 1 871 24
	movq	-30776(%rbp), %rax
	movl	(%rax), %eax
	leaq	-30752(%rbp), %rsi
	movl	$0, %ecx
	movl	$30720, %edx
	movl	%eax, %edi
	call	recv@PLT
	.loc 1 871 24 is_stmt 0 discriminator 1
	movl	%eax, -30780(%rbp)
	cmpl	$0, -30780(%rbp)
	jns	.L180
	call	__errno_location@PLT
	movl	(%rax), %eax
	movl	%eax, %edi
	call	strerror@PLT
	movq	%rax, %rbx
	.loc 1 871 24 discriminator 2
	call	__errno_location@PLT
	.loc 1 871 24 discriminator 3
	movl	(%rax), %eax
	movq	%rbx, %rdx
	movl	%eax, %esi
	leaq	.LC15(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 871 24
	jmp	.L173
.L180:
	.loc 1 871 24 discriminator 2
	cmpl	$0, -30780(%rbp)
	jne	.L181
	.loc 1 871 24 discriminator 3
	movq	-30776(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 871 24 discriminator 1
	leaq	.LC16(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 871 24
	jmp	.L173
.L181:
	.loc 1 871 24 discriminator 4
	movl	-30780(%rbp), %eax
.LBE44:
	.loc 1 871 22 is_stmt 1 discriminator 4
	movl	%eax, -30796(%rbp)
	.loc 1 872 17
	leaq	-30752(%rbp), %rax
	movl	$7, %edx
	leaq	.LC17(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 872 16 discriminator 1
	testl	%eax, %eax
	jne	.L182
	.loc 1 874 17
	leaq	.LC26(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 875 17
	leaq	.LC99(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 876 17
	movq	-30824(%rbp), %rax
	movq	16(%rax), %rax
	movq	%rax, %rsi
	leaq	.LC28(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 877 17
	leaq	.LC100(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 878 17
	leaq	.LC30(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L182:
	.loc 1 880 17
	leaq	-30752(%rbp), %rax
	movl	$8, %edx
	leaq	.LC77(%rip), %rcx
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	strncmp@PLT
	.loc 1 880 16 discriminator 1
	testl	%eax, %eax
	jne	.L183
	.loc 1 882 17
	leaq	.LC101(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L183:
.LBE41:
	.loc 1 858 63 discriminator 2
	addl	$1, -30800(%rbp)
.L176:
	.loc 1 858 27 discriminator 1
	cmpl	$1, -30800(%rbp)
	jle	.L184
.LBE40:
	.loc 1 887 5
	jmp	.L173
.L187:
	.loc 1 854 13
	nop
.L173:
	.loc 1 888 9
	movq	-30776(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
	.loc 1 889 17
	movq	-30776(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -30776(%rbp)
	.loc 1 890 10
	addl	$1, -30808(%rbp)
.L171:
.LBE39:
	.loc 1 851 12
	cmpq	$0, -30776(%rbp)
	jne	.L185
	.loc 1 892 1
	nop
	movq	-24(%rbp), %rax
	subq	%fs:40, %rax
	je	.L186
	call	__stack_chk_fail@PLT
.L186:
	movq	-8(%rbp), %rbx
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE331:
	.size	delete_file, .-delete_file
	.section	.rodata
	.align 8
.LC102:
	.string	"\033[36m    DFS SERVER STATUS CHECK\n\033[0m"
	.align 8
.LC103:
	.string	"\033[36m[*] CHECKING INDIVIDUAL SERVER STATUS\n\n\033[0m"
	.align 8
.LC104:
	.string	"\033[36m[*] Server %d (%s:%s): \033[0m"
.LC105:
	.string	"\033[31mOFFLINE\n\033[0m"
	.align 8
.LC106:
	.string	"\033[31m    Unable to establish connection\n\n\033[0m"
.LC107:
	.string	"\033[32mONLINE\n\033[0m"
	.align 8
.LC108:
	.string	"\033[32m    Connection established successfully\n\033[0m"
	.align 8
.LC110:
	.string	"\033[32m    Response time: %.2f ms\n\n\033[0m"
	.align 8
.LC111:
	.string	"\033[36m    SYSTEM STATUS SUMMARY\n\033[0m"
.LC112:
	.string	"Servers available: %d/%d "
	.align 8
.LC113:
	.string	"\033[32m(All servers online)\n\033[0m"
	.align 8
.LC114:
	.string	"\033[33m(System operational with reduced redundancy)\n\033[0m"
	.align 8
.LC115:
	.string	"\033[33m(System partially operational - limited functionality)\n\033[0m"
	.align 8
.LC116:
	.string	"\033[31m(System offline - no servers available)\n\033[0m"
	.align 8
.LC117:
	.string	"\nSystem capabilities with %d/%d servers:\n"
.LC118:
	.string	"\033[32m  [\342\234\223] \033[0m"
.LC119:
	.string	"\033[31m  [\342\234\227] \033[0m"
.LC120:
	.string	"LIST - View available files"
.LC121:
	.string	"GET - Download complete files"
.LC122:
	.string	"full"
.LC123:
	.string	"partial"
	.align 8
.LC124:
	.string	"PUT - Upload files (with %s redundancy)\n"
	.align 8
.LC125:
	.string	"DELETE - Remove files (may be incomplete with partial servers)\n"
	.text
	.globl	server_info
	.type	server_info, @function
server_info:
.LFB332:
	.loc 1 895 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$32, %rsp
	movq	%rdi, -24(%rbp)
	.loc 1 896 5
	leaq	.LC52(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 897 5
	leaq	.LC102(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 898 5
	leaq	.LC54(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 900 9
	movl	$0, -16(%rbp)
	.loc 1 901 9
	movl	$0, -12(%rbp)
	.loc 1 902 22
	movq	-24(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -8(%rbp)
	.loc 1 905 36
	movq	-24(%rbp), %rax
	movl	56(%rax), %eax
	cltq
	.loc 1 905 27
	salq	$2, %rax
	movq	%rax, %rdi
	call	malloc@PLT
	movq	%rax, %rdx
	.loc 1 905 25 discriminator 1
	movq	-24(%rbp), %rax
	movq	%rdx, 48(%rax)
	.loc 1 908 5
	leaq	.LC103(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 911 11
	jmp	.L189
.L193:
	.loc 1 913 9
	movq	-8(%rbp), %rax
	movq	8(%rax), %rdx
	movq	-8(%rbp), %rax
	movq	16(%rax), %rax
	movl	-16(%rbp), %ecx
	leal	1(%rcx), %esi
	movq	%rdx, %rcx
	movq	%rax, %rdx
	leaq	.LC104(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 916 13
	movl	-16(%rbp), %edx
	movq	-8(%rbp), %rcx
	movq	-24(%rbp), %rax
	movq	%rcx, %rsi
	movq	%rax, %rdi
	call	connect_server
	.loc 1 916 12 discriminator 1
	testl	%eax, %eax
	jns	.L190
	.loc 1 917 13
	leaq	.LC105(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 918 13
	leaq	.LC106(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L191
.L190:
	.loc 1 920 27
	addl	$1, -12(%rbp)
	.loc 1 921 13
	leaq	.LC107(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 922 13
	leaq	.LC108(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 925 72
	call	rand@PLT
	.loc 1 925 79 discriminator 1
	movslq	%eax, %rdx
	imulq	$1374389535, %rdx, %rdx
	shrq	$32, %rdx
	sarl	$5, %edx
	movl	%eax, %ecx
	sarl	$31, %ecx
	subl	%ecx, %edx
	imull	$100, %edx, %ecx
	subl	%ecx, %eax
	movl	%eax, %edx
	.loc 1 925 13 discriminator 1
	pxor	%xmm1, %xmm1
	cvtsi2sdl	%edx, %xmm1
	movsd	.LC109(%rip), %xmm0
	addsd	%xmm0, %xmm1
	movq	%xmm1, %rax
	movq	%rax, %xmm0
	leaq	.LC110(%rip), %rax
	movq	%rax, %rdi
	movl	$1, %eax
	call	printf@PLT
.L191:
	.loc 1 929 20
	movq	-8(%rbp), %rax
	movl	(%rax), %eax
	.loc 1 929 12
	testl	%eax, %eax
	js	.L192
	.loc 1 930 13
	movq	-8(%rbp), %rax
	movl	(%rax), %eax
	movl	%eax, %edi
	call	close@PLT
.L192:
	.loc 1 932 17
	movq	-8(%rbp), %rax
	movq	32(%rax), %rax
	movq	%rax, -8(%rbp)
	.loc 1 933 10
	addl	$1, -16(%rbp)
.L189:
	.loc 1 911 12
	cmpq	$0, -8(%rbp)
	jne	.L193
	.loc 1 937 5
	leaq	.LC52(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 938 5
	leaq	.LC111(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 939 5
	leaq	.LC54(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 942 5
	movq	-24(%rbp), %rax
	movl	56(%rax), %edx
	movl	-12(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC112(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 944 29
	movq	-24(%rbp), %rax
	movl	56(%rax), %eax
	.loc 1 944 8
	cmpl	%eax, -12(%rbp)
	jne	.L194
	.loc 1 945 9
	leaq	.LC113(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L195
.L194:
	.loc 1 946 15
	cmpl	$1, -12(%rbp)
	jle	.L196
	.loc 1 947 9
	leaq	.LC114(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L195
.L196:
	.loc 1 948 15
	cmpl	$0, -12(%rbp)
	jle	.L197
	.loc 1 949 9
	leaq	.LC115(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L195
.L197:
	.loc 1 951 9
	leaq	.LC116(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L195:
	.loc 1 955 5
	movq	-24(%rbp), %rax
	movl	56(%rax), %edx
	movl	-12(%rbp), %eax
	movl	%eax, %esi
	leaq	.LC117(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 957 8
	cmpl	$0, -12(%rbp)
	jle	.L198
	.loc 1 958 9
	leaq	.LC118(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L199
.L198:
	.loc 1 960 9
	leaq	.LC119(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L199:
	.loc 1 962 5
	leaq	.LC120(%rip), %rax
	movq	%rax, %rdi
	call	puts@PLT
	.loc 1 964 8
	cmpl	$3, -12(%rbp)
	jle	.L200
	.loc 1 965 9
	leaq	.LC118(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L201
.L200:
	.loc 1 967 9
	leaq	.LC119(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L201:
	.loc 1 969 5
	leaq	.LC121(%rip), %rax
	movq	%rax, %rdi
	call	puts@PLT
	.loc 1 971 8
	cmpl	$0, -12(%rbp)
	jle	.L202
	.loc 1 972 9
	leaq	.LC118(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L203
.L202:
	.loc 1 974 9
	leaq	.LC119(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L203:
	.loc 1 976 5
	cmpl	$3, -12(%rbp)
	jle	.L204
	.loc 1 976 5 is_stmt 0 discriminator 1
	leaq	.LC122(%rip), %rax
	jmp	.L205
.L204:
	.loc 1 976 5 discriminator 2
	leaq	.LC123(%rip), %rax
.L205:
	.loc 1 976 5 discriminator 4
	movq	%rax, %rsi
	leaq	.LC124(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 979 8 is_stmt 1
	cmpl	$0, -12(%rbp)
	jle	.L206
	.loc 1 980 9
	leaq	.LC118(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	jmp	.L207
.L206:
	.loc 1 982 9
	leaq	.LC119(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
.L207:
	.loc 1 984 5
	leaq	.LC125(%rip), %rax
	movq	%rax, %rdi
	call	puts@PLT
	.loc 1 986 5
	leaq	.LC54(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 989 12
	movq	-24(%rbp), %rax
	movq	48(%rax), %rax
	.loc 1 989 5
	movq	%rax, %rdi
	call	free@PLT
	.loc 1 990 25
	movq	-24(%rbp), %rax
	movq	$0, 48(%rax)
	.loc 1 991 1
	nop
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE332:
	.size	server_info, .-server_info
	.section	.rodata
.LC126:
	.string	"SERVER INFO"
.LC127:
	.string	"UNKNOWN"
.LC128:
	.string	"DELETE"
.LC129:
	.string	"LIST"
.LC130:
	.string	"GET"
.LC131:
	.string	"PUT"
.LC132:
	.string	"\033[36m    OPERATION: %s\n\033[0m"
	.text
	.globl	handle_req
	.type	handle_req, @function
handle_req:
.LFB333:
	.loc 1 1010 1
	.cfi_startproc
	endbr64
	pushq	%rbp
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
	movq	%rsp, %rbp
	.cfi_def_cfa_register 6
	subq	$16, %rsp
	movq	%rdi, -8(%rbp)
	.loc 1 1012 5
	leaq	.LC52(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 1014 14
	movq	-8(%rbp), %rax
	movl	24(%rax), %eax
	.loc 1 1013 5
	cmpl	$1, %eax
	je	.L209
	.loc 1 1015 14
	movq	-8(%rbp), %rax
	movl	24(%rax), %eax
	.loc 1 1015 43
	testl	%eax, %eax
	je	.L210
	.loc 1 1016 14
	movq	-8(%rbp), %rax
	movl	24(%rax), %eax
	.loc 1 1016 43
	cmpl	$3, %eax
	je	.L211
	.loc 1 1017 14
	movq	-8(%rbp), %rax
	movl	24(%rax), %eax
	.loc 1 1017 49
	cmpl	$2, %eax
	je	.L212
	.loc 1 1018 14
	movq	-8(%rbp), %rax
	movl	24(%rax), %eax
	.loc 1 1018 59
	cmpl	$5, %eax
	jne	.L213
	.loc 1 1018 59 is_stmt 0 discriminator 1
	leaq	.LC126(%rip), %rax
	jmp	.L218
.L213:
	.loc 1 1018 59 discriminator 2
	leaq	.LC127(%rip), %rax
	jmp	.L218
.L212:
	.loc 1 1017 49 is_stmt 1 discriminator 1
	leaq	.LC128(%rip), %rax
	jmp	.L218
.L211:
	.loc 1 1016 43 discriminator 1
	leaq	.LC129(%rip), %rax
	jmp	.L218
.L210:
	.loc 1 1015 43 discriminator 1
	leaq	.LC130(%rip), %rax
	jmp	.L218
.L209:
	.loc 1 1013 5 discriminator 1
	leaq	.LC131(%rip), %rax
.L218:
	.loc 1 1013 5 is_stmt 0 discriminator 3
	movq	%rax, %rsi
	leaq	.LC132(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 1019 5 is_stmt 1
	leaq	.LC54(%rip), %rax
	movq	%rax, %rdi
	movl	$0, %eax
	call	printf@PLT
	.loc 1 1022 15
	movq	-8(%rbp), %rax
	movl	24(%rax), %eax
	.loc 1 1022 5
	cmpl	$5, %eax
	ja	.L229
	movl	%eax, %eax
	leaq	0(,%rax,4), %rdx
	leaq	.L221(%rip), %rax
	movl	(%rdx,%rax), %eax
	cltq
	leaq	.L221(%rip), %rdx
	addq	%rdx, %rax
	notrack jmp	*%rax
	.section	.rodata
	.align 4
	.align 4
.L221:
	.long	.L226-.L221
	.long	.L225-.L221
	.long	.L224-.L221
	.long	.L223-.L221
	.long	.L222-.L221
	.long	.L220-.L221
	.text
.L223:
	.loc 1 1025 9
	movq	-8(%rbp), %rax
	movq	%rax, %rdi
	call	list_file
	.loc 1 1026 9
	jmp	.L227
.L226:
	.loc 1 1028 9
	movq	-8(%rbp), %rax
	movq	%rax, %rdi
	call	get_file
	.loc 1 1029 9
	jmp	.L227
.L225:
	.loc 1 1031 9
	movq	-8(%rbp), %rax
	movq	%rax, %rdi
	call	put_file
	.loc 1 1032 9
	jmp	.L227
.L224:
	.loc 1 1034 9
	movq	-8(%rbp), %rax
	movq	%rax, %rdi
	call	delete_file
	.loc 1 1035 9
	jmp	.L227
.L222:
	.loc 1 1037 9
	movl	$0, %edi
	call	exit@PLT
.L220:
	.loc 1 1040 9
	movq	-8(%rbp), %rax
	movq	%rax, %rdi
	call	server_info
	.loc 1 1041 9
	jmp	.L227
.L229:
	.loc 1 1043 9
	nop
.L227:
	.loc 1 1046 12
	movl	$0, %eax
	.loc 1 1047 1
	leave
	.cfi_def_cfa 7, 8
	ret
	.cfi_endproc
.LFE333:
	.size	handle_req, .-handle_req
	.section	.rodata
	.align 4
.LC48:
	.long	1120403456
	.align 8
.LC109:
	.long	0
	.long	1076756480
	.text
.Letext0:
	.file 2 "/usr/lib/gcc/x86_64-linux-gnu/14/include/stddef.h"
	.file 3 "/usr/include/x86_64-linux-gnu/bits/types.h"
	.file 4 "/usr/include/x86_64-linux-gnu/bits/types/struct_FILE.h"
	.file 5 "/usr/include/x86_64-linux-gnu/bits/types/FILE.h"
	.file 6 "/usr/include/stdio.h"
	.file 7 "/usr/include/x86_64-linux-gnu/bits/thread-shared-types.h"
	.file 8 "/usr/include/x86_64-linux-gnu/bits/struct_mutex.h"
	.file 9 "/usr/include/x86_64-linux-gnu/bits/pthreadtypes.h"
	.file 10 "/usr/include/x86_64-linux-gnu/bits/stdint-uintn.h"
	.file 11 "/usr/include/unistd.h"
	.file 12 "/usr/include/x86_64-linux-gnu/bits/sockaddr.h"
	.file 13 "/usr/include/x86_64-linux-gnu/bits/socket.h"
	.file 14 "/usr/include/netinet/in.h"
	.file 15 "/usr/include/x86_64-linux-gnu/sys/socket.h"
	.file 16 "/usr/include/netdb.h"
	.file 17 "/usr/include/openssl/types.h"
	.file 18 "./common.h"
	.file 19 "/usr/include/string.h"
	.file 20 "/usr/include/stdlib.h"
	.file 21 "/usr/include/arpa/inet.h"
	.file 22 "/usr/include/x86_64-linux-gnu/bits/unistd_ext.h"
	.file 23 "/usr/include/errno.h"
	.file 24 "/usr/include/openssl/evp.h"
	.file 25 "/usr/include/x86_64-linux-gnu/bits/socket_type.h"
	.section	.debug_info,"",@progbits
.Ldebug_info0:
	.long	0x1b0f
	.value	0x5
	.byte	0x1
	.byte	0x8
	.long	.Ldebug_abbrev0
	.uleb128 0x27
	.long	.LASF260
	.byte	0x1d
	.long	.LASF0
	.long	.LASF1
	.quad	.Ltext0
	.quad	.Letext0-.Ltext0
	.long	.Ldebug_line0
	.uleb128 0x5
	.long	.LASF7
	.byte	0x2
	.byte	0xd6
	.byte	0x17
	.long	0x3a
	.uleb128 0xd
	.byte	0x8
	.byte	0x7
	.long	.LASF2
	.uleb128 0xd
	.byte	0x4
	.byte	0x7
	.long	.LASF3
	.uleb128 0x28
	.byte	0x8
	.uleb128 0x12
	.long	0x48
	.uleb128 0xd
	.byte	0x1
	.byte	0x8
	.long	.LASF4
	.uleb128 0xd
	.byte	0x2
	.byte	0x7
	.long	.LASF5
	.uleb128 0xd
	.byte	0x1
	.byte	0x6
	.long	.LASF6
	.uleb128 0x5
	.long	.LASF8
	.byte	0x3
	.byte	0x26
	.byte	0x17
	.long	0x4f
	.uleb128 0xd
	.byte	0x2
	.byte	0x5
	.long	.LASF9
	.uleb128 0x5
	.long	.LASF10
	.byte	0x3
	.byte	0x28
	.byte	0x1c
	.long	0x56
	.uleb128 0x29
	.byte	0x4
	.byte	0x5
	.string	"int"
	.uleb128 0x5
	.long	.LASF11
	.byte	0x3
	.byte	0x2a
	.byte	0x16
	.long	0x41
	.uleb128 0xd
	.byte	0x8
	.byte	0x5
	.long	.LASF12
	.uleb128 0x5
	.long	.LASF13
	.byte	0x3
	.byte	0x98
	.byte	0x19
	.long	0x96
	.uleb128 0x5
	.long	.LASF14
	.byte	0x3
	.byte	0x99
	.byte	0x1b
	.long	0x96
	.uleb128 0x5
	.long	.LASF15
	.byte	0x3
	.byte	0x9a
	.byte	0x19
	.long	0x83
	.uleb128 0x5
	.long	.LASF16
	.byte	0x3
	.byte	0xc2
	.byte	0x1b
	.long	0x96
	.uleb128 0x4
	.long	0xd7
	.uleb128 0x12
	.long	0xcd
	.uleb128 0xd
	.byte	0x1
	.byte	0x6
	.long	.LASF17
	.uleb128 0x1c
	.long	0xd7
	.uleb128 0x5
	.long	.LASF18
	.byte	0x3
	.byte	0xd2
	.byte	0x17
	.long	0x41
	.uleb128 0x11
	.long	.LASF55
	.byte	0xd8
	.byte	0x4
	.byte	0x31
	.byte	0x8
	.long	0x276
	.uleb128 0x2
	.long	.LASF19
	.byte	0x4
	.byte	0x33
	.byte	0x7
	.long	0x83
	.byte	0
	.uleb128 0x2
	.long	.LASF20
	.byte	0x4
	.byte	0x36
	.byte	0x9
	.long	0xcd
	.byte	0x8
	.uleb128 0x2
	.long	.LASF21
	.byte	0x4
	.byte	0x37
	.byte	0x9
	.long	0xcd
	.byte	0x10
	.uleb128 0x2
	.long	.LASF22
	.byte	0x4
	.byte	0x38
	.byte	0x9
	.long	0xcd
	.byte	0x18
	.uleb128 0x2
	.long	.LASF23
	.byte	0x4
	.byte	0x39
	.byte	0x9
	.long	0xcd
	.byte	0x20
	.uleb128 0x2
	.long	.LASF24
	.byte	0x4
	.byte	0x3a
	.byte	0x9
	.long	0xcd
	.byte	0x28
	.uleb128 0x2
	.long	.LASF25
	.byte	0x4
	.byte	0x3b
	.byte	0x9
	.long	0xcd
	.byte	0x30
	.uleb128 0x2
	.long	.LASF26
	.byte	0x4
	.byte	0x3c
	.byte	0x9
	.long	0xcd
	.byte	0x38
	.uleb128 0x2
	.long	.LASF27
	.byte	0x4
	.byte	0x3d
	.byte	0x9
	.long	0xcd
	.byte	0x40
	.uleb128 0x2
	.long	.LASF28
	.byte	0x4
	.byte	0x40
	.byte	0x9
	.long	0xcd
	.byte	0x48
	.uleb128 0x2
	.long	.LASF29
	.byte	0x4
	.byte	0x41
	.byte	0x9
	.long	0xcd
	.byte	0x50
	.uleb128 0x2
	.long	.LASF30
	.byte	0x4
	.byte	0x42
	.byte	0x9
	.long	0xcd
	.byte	0x58
	.uleb128 0x2
	.long	.LASF31
	.byte	0x4
	.byte	0x44
	.byte	0x16
	.long	0x28f
	.byte	0x60
	.uleb128 0x2
	.long	.LASF32
	.byte	0x4
	.byte	0x46
	.byte	0x14
	.long	0x294
	.byte	0x68
	.uleb128 0x2
	.long	.LASF33
	.byte	0x4
	.byte	0x48
	.byte	0x7
	.long	0x83
	.byte	0x70
	.uleb128 0x2
	.long	.LASF34
	.byte	0x4
	.byte	0x49
	.byte	0x7
	.long	0x83
	.byte	0x74
	.uleb128 0x2
	.long	.LASF35
	.byte	0x4
	.byte	0x4a
	.byte	0xb
	.long	0x9d
	.byte	0x78
	.uleb128 0x2
	.long	.LASF36
	.byte	0x4
	.byte	0x4d
	.byte	0x12
	.long	0x56
	.byte	0x80
	.uleb128 0x2
	.long	.LASF37
	.byte	0x4
	.byte	0x4e
	.byte	0xf
	.long	0x5d
	.byte	0x82
	.uleb128 0x2
	.long	.LASF38
	.byte	0x4
	.byte	0x4f
	.byte	0x8
	.long	0x299
	.byte	0x83
	.uleb128 0x2
	.long	.LASF39
	.byte	0x4
	.byte	0x51
	.byte	0xf
	.long	0x2a9
	.byte	0x88
	.uleb128 0x2
	.long	.LASF40
	.byte	0x4
	.byte	0x59
	.byte	0xd
	.long	0xa9
	.byte	0x90
	.uleb128 0x2
	.long	.LASF41
	.byte	0x4
	.byte	0x5b
	.byte	0x17
	.long	0x2b3
	.byte	0x98
	.uleb128 0x2
	.long	.LASF42
	.byte	0x4
	.byte	0x5c
	.byte	0x19
	.long	0x2bd
	.byte	0xa0
	.uleb128 0x2
	.long	.LASF43
	.byte	0x4
	.byte	0x5d
	.byte	0x14
	.long	0x294
	.byte	0xa8
	.uleb128 0x2
	.long	.LASF44
	.byte	0x4
	.byte	0x5e
	.byte	0x9
	.long	0x48
	.byte	0xb0
	.uleb128 0x2
	.long	.LASF45
	.byte	0x4
	.byte	0x5f
	.byte	0x15
	.long	0x2c2
	.byte	0xb8
	.uleb128 0x2
	.long	.LASF46
	.byte	0x4
	.byte	0x60
	.byte	0x7
	.long	0x83
	.byte	0xc0
	.uleb128 0x2
	.long	.LASF47
	.byte	0x4
	.byte	0x62
	.byte	0x8
	.long	0x2c7
	.byte	0xc4
	.byte	0
	.uleb128 0x5
	.long	.LASF48
	.byte	0x5
	.byte	0x7
	.byte	0x19
	.long	0xef
	.uleb128 0x2a
	.long	.LASF261
	.byte	0x4
	.byte	0x2b
	.byte	0xe
	.uleb128 0x14
	.long	.LASF49
	.uleb128 0x4
	.long	0x28a
	.uleb128 0x4
	.long	0xef
	.uleb128 0xe
	.long	0xd7
	.long	0x2a9
	.uleb128 0xf
	.long	0x3a
	.byte	0
	.byte	0
	.uleb128 0x4
	.long	0x282
	.uleb128 0x14
	.long	.LASF50
	.uleb128 0x4
	.long	0x2ae
	.uleb128 0x14
	.long	.LASF51
	.uleb128 0x4
	.long	0x2b8
	.uleb128 0x4
	.long	0x294
	.uleb128 0xe
	.long	0xd7
	.long	0x2d7
	.uleb128 0xf
	.long	0x3a
	.byte	0x13
	.byte	0
	.uleb128 0x4
	.long	0xde
	.uleb128 0x12
	.long	0x2d7
	.uleb128 0x5
	.long	.LASF52
	.byte	0x6
	.byte	0x4e
	.byte	0x13
	.long	0xc1
	.uleb128 0x4
	.long	0x276
	.uleb128 0x12
	.long	0x2ed
	.uleb128 0x2b
	.long	.LASF262
	.byte	0x6
	.byte	0x97
	.byte	0xe
	.long	0x2ed
	.uleb128 0xd
	.byte	0x8
	.byte	0x5
	.long	.LASF53
	.uleb128 0xd
	.byte	0x8
	.byte	0x7
	.long	.LASF54
	.uleb128 0x11
	.long	.LASF56
	.byte	0x10
	.byte	0x7
	.byte	0x33
	.byte	0x10
	.long	0x339
	.uleb128 0x2
	.long	.LASF57
	.byte	0x7
	.byte	0x35
	.byte	0x23
	.long	0x339
	.byte	0
	.uleb128 0x2
	.long	.LASF58
	.byte	0x7
	.byte	0x36
	.byte	0x23
	.long	0x339
	.byte	0x8
	.byte	0
	.uleb128 0x4
	.long	0x311
	.uleb128 0x5
	.long	.LASF59
	.byte	0x7
	.byte	0x37
	.byte	0x3
	.long	0x311
	.uleb128 0x11
	.long	.LASF60
	.byte	0x28
	.byte	0x8
	.byte	0x16
	.byte	0x8
	.long	0x3c0
	.uleb128 0x2
	.long	.LASF61
	.byte	0x8
	.byte	0x18
	.byte	0x7
	.long	0x83
	.byte	0
	.uleb128 0x2
	.long	.LASF62
	.byte	0x8
	.byte	0x19
	.byte	0x10
	.long	0x41
	.byte	0x4
	.uleb128 0x2
	.long	.LASF63
	.byte	0x8
	.byte	0x1a
	.byte	0x7
	.long	0x83
	.byte	0x8
	.uleb128 0x2
	.long	.LASF64
	.byte	0x8
	.byte	0x1c
	.byte	0x10
	.long	0x41
	.byte	0xc
	.uleb128 0x2
	.long	.LASF65
	.byte	0x8
	.byte	0x20
	.byte	0x7
	.long	0x83
	.byte	0x10
	.uleb128 0x2
	.long	.LASF66
	.byte	0x8
	.byte	0x22
	.byte	0x9
	.long	0x70
	.byte	0x14
	.uleb128 0x2
	.long	.LASF67
	.byte	0x8
	.byte	0x23
	.byte	0x9
	.long	0x70
	.byte	0x16
	.uleb128 0x2
	.long	.LASF68
	.byte	0x8
	.byte	0x24
	.byte	0x14
	.long	0x33e
	.byte	0x18
	.byte	0
	.uleb128 0x24
	.byte	0x28
	.byte	0x9
	.byte	0x43
	.byte	0x9
	.long	0x3ee
	.uleb128 0x15
	.long	.LASF69
	.byte	0x9
	.byte	0x45
	.byte	0x1c
	.long	0x34a
	.uleb128 0x15
	.long	.LASF70
	.byte	0x9
	.byte	0x46
	.byte	0x8
	.long	0x3ee
	.uleb128 0x15
	.long	.LASF71
	.byte	0x9
	.byte	0x47
	.byte	0xc
	.long	0x96
	.byte	0
	.uleb128 0xe
	.long	0xd7
	.long	0x3fe
	.uleb128 0xf
	.long	0x3a
	.byte	0x27
	.byte	0
	.uleb128 0x5
	.long	.LASF72
	.byte	0x9
	.byte	0x48
	.byte	0x3
	.long	0x3c0
	.uleb128 0x4
	.long	0x414
	.uleb128 0x12
	.long	0x40a
	.uleb128 0x2c
	.uleb128 0x5
	.long	.LASF73
	.byte	0xa
	.byte	0x18
	.byte	0x13
	.long	0x64
	.uleb128 0x5
	.long	.LASF74
	.byte	0xa
	.byte	0x19
	.byte	0x14
	.long	0x77
	.uleb128 0x5
	.long	.LASF75
	.byte	0xa
	.byte	0x1a
	.byte	0x14
	.long	0x8a
	.uleb128 0x25
	.long	.LASF76
	.byte	0xb
	.value	0x112
	.byte	0x15
	.long	0xe3
	.uleb128 0x4
	.long	0xcd
	.uleb128 0x12
	.long	0x446
	.uleb128 0x2d
	.long	.LASF263
	.byte	0x7
	.byte	0x4
	.long	0x41
	.byte	0x19
	.byte	0x18
	.byte	0x6
	.long	0x49d
	.uleb128 0x10
	.long	.LASF77
	.byte	0x1
	.uleb128 0x10
	.long	.LASF78
	.byte	0x2
	.uleb128 0x10
	.long	.LASF79
	.byte	0x3
	.uleb128 0x10
	.long	.LASF80
	.byte	0x4
	.uleb128 0x10
	.long	.LASF81
	.byte	0x5
	.uleb128 0x10
	.long	.LASF82
	.byte	0x6
	.uleb128 0x10
	.long	.LASF83
	.byte	0xa
	.uleb128 0x2e
	.long	.LASF84
	.long	0x80000
	.uleb128 0x2f
	.long	.LASF85
	.value	0x800
	.byte	0
	.uleb128 0x5
	.long	.LASF86
	.byte	0xc
	.byte	0x1c
	.byte	0x1c
	.long	0x56
	.uleb128 0x11
	.long	.LASF87
	.byte	0x10
	.byte	0xd
	.byte	0xb8
	.byte	0x27
	.long	0x4d1
	.uleb128 0x2
	.long	.LASF88
	.byte	0xd
	.byte	0xba
	.byte	0x5
	.long	0x49d
	.byte	0
	.uleb128 0x2
	.long	.LASF89
	.byte	0xd
	.byte	0xbb
	.byte	0xa
	.long	0x4d1
	.byte	0x2
	.byte	0
	.uleb128 0xe
	.long	0xd7
	.long	0x4e1
	.uleb128 0xf
	.long	0x3a
	.byte	0xd
	.byte	0
	.uleb128 0x4
	.long	0x4a9
	.uleb128 0x11
	.long	.LASF90
	.byte	0x10
	.byte	0xe
	.byte	0xf7
	.byte	0x27
	.long	0x528
	.uleb128 0x2
	.long	.LASF91
	.byte	0xe
	.byte	0xf9
	.byte	0x5
	.long	0x49d
	.byte	0
	.uleb128 0x2
	.long	.LASF92
	.byte	0xe
	.byte	0xfa
	.byte	0xf
	.long	0x5b5
	.byte	0x2
	.uleb128 0x2
	.long	.LASF93
	.byte	0xe
	.byte	0xfb
	.byte	0x14
	.long	0x59a
	.byte	0x4
	.uleb128 0x2
	.long	.LASF94
	.byte	0xe
	.byte	0xfe
	.byte	0x13
	.long	0x63a
	.byte	0x8
	.byte	0
	.uleb128 0x1d
	.long	.LASF95
	.byte	0x1c
	.byte	0xe
	.value	0x108
	.byte	0x27
	.long	0x57d
	.uleb128 0xb
	.long	.LASF96
	.byte	0xe
	.value	0x10a
	.byte	0x5
	.long	0x49d
	.byte	0
	.uleb128 0xb
	.long	.LASF97
	.byte	0xe
	.value	0x10b
	.byte	0xf
	.long	0x5b5
	.byte	0x2
	.uleb128 0xb
	.long	.LASF98
	.byte	0xe
	.value	0x10c
	.byte	0xe
	.long	0x42d
	.byte	0x4
	.uleb128 0xb
	.long	.LASF99
	.byte	0xe
	.value	0x10d
	.byte	0x15
	.long	0x61f
	.byte	0x8
	.uleb128 0xb
	.long	.LASF100
	.byte	0xe
	.value	0x10e
	.byte	0xe
	.long	0x42d
	.byte	0x18
	.byte	0
	.uleb128 0x5
	.long	.LASF101
	.byte	0xf
	.byte	0x54
	.byte	0xa
	.long	0x589
	.uleb128 0x30
	.byte	0x8
	.byte	0xf
	.byte	0x53
	.byte	0x9
	.uleb128 0x5
	.long	.LASF102
	.byte	0xe
	.byte	0x1e
	.byte	0x12
	.long	0x42d
	.uleb128 0x11
	.long	.LASF103
	.byte	0x4
	.byte	0xe
	.byte	0x1f
	.byte	0x8
	.long	0x5b5
	.uleb128 0x2
	.long	.LASF104
	.byte	0xe
	.byte	0x21
	.byte	0xf
	.long	0x58e
	.byte	0
	.byte	0
	.uleb128 0x5
	.long	.LASF105
	.byte	0xe
	.byte	0x7d
	.byte	0x12
	.long	0x421
	.uleb128 0x24
	.byte	0x10
	.byte	0xe
	.byte	0xdf
	.byte	0x5
	.long	0x5ef
	.uleb128 0x15
	.long	.LASF106
	.byte	0xe
	.byte	0xe1
	.byte	0xa
	.long	0x5ef
	.uleb128 0x15
	.long	.LASF107
	.byte	0xe
	.byte	0xe2
	.byte	0xb
	.long	0x5ff
	.uleb128 0x15
	.long	.LASF108
	.byte	0xe
	.byte	0xe3
	.byte	0xb
	.long	0x60f
	.byte	0
	.uleb128 0xe
	.long	0x415
	.long	0x5ff
	.uleb128 0xf
	.long	0x3a
	.byte	0xf
	.byte	0
	.uleb128 0xe
	.long	0x421
	.long	0x60f
	.uleb128 0xf
	.long	0x3a
	.byte	0x7
	.byte	0
	.uleb128 0xe
	.long	0x42d
	.long	0x61f
	.uleb128 0xf
	.long	0x3a
	.byte	0x3
	.byte	0
	.uleb128 0x11
	.long	.LASF109
	.byte	0x10
	.byte	0xe
	.byte	0xdd
	.byte	0x8
	.long	0x63a
	.uleb128 0x2
	.long	.LASF110
	.byte	0xe
	.byte	0xe4
	.byte	0x9
	.long	0x5c1
	.byte	0
	.byte	0
	.uleb128 0xe
	.long	0x4f
	.long	0x64a
	.uleb128 0xf
	.long	0x3a
	.byte	0x7
	.byte	0
	.uleb128 0x1d
	.long	.LASF111
	.byte	0x30
	.byte	0x10
	.value	0x235
	.byte	0x8
	.long	0x6c9
	.uleb128 0xb
	.long	.LASF112
	.byte	0x10
	.value	0x237
	.byte	0x7
	.long	0x83
	.byte	0
	.uleb128 0xb
	.long	.LASF113
	.byte	0x10
	.value	0x238
	.byte	0x7
	.long	0x83
	.byte	0x4
	.uleb128 0xb
	.long	.LASF114
	.byte	0x10
	.value	0x239
	.byte	0x7
	.long	0x83
	.byte	0x8
	.uleb128 0xb
	.long	.LASF115
	.byte	0x10
	.value	0x23a
	.byte	0x7
	.long	0x83
	.byte	0xc
	.uleb128 0xb
	.long	.LASF116
	.byte	0x10
	.value	0x23b
	.byte	0xd
	.long	0x439
	.byte	0x10
	.uleb128 0xb
	.long	.LASF117
	.byte	0x10
	.value	0x23c
	.byte	0x14
	.long	0x4e1
	.byte	0x18
	.uleb128 0xb
	.long	.LASF118
	.byte	0x10
	.value	0x23d
	.byte	0x9
	.long	0xcd
	.byte	0x20
	.uleb128 0xb
	.long	.LASF119
	.byte	0x10
	.value	0x23e
	.byte	0x14
	.long	0x6ce
	.byte	0x28
	.byte	0
	.uleb128 0x1c
	.long	0x64a
	.uleb128 0x4
	.long	0x64a
	.uleb128 0x4
	.long	0x6c9
	.uleb128 0x12
	.long	0x6d3
	.uleb128 0xd
	.byte	0x10
	.byte	0x5
	.long	.LASF120
	.uleb128 0xd
	.byte	0x10
	.byte	0x7
	.long	.LASF121
	.uleb128 0xd
	.byte	0x10
	.byte	0x4
	.long	.LASF122
	.uleb128 0x5
	.long	.LASF123
	.byte	0x11
	.byte	0x6a
	.byte	0x1a
	.long	0x703
	.uleb128 0x1c
	.long	0x6f2
	.uleb128 0x14
	.long	.LASF124
	.uleb128 0x5
	.long	.LASF125
	.byte	0x11
	.byte	0x6b
	.byte	0x1e
	.long	0x714
	.uleb128 0x14
	.long	.LASF126
	.uleb128 0x5
	.long	.LASF127
	.byte	0x11
	.byte	0xba
	.byte	0x1a
	.long	0x725
	.uleb128 0x14
	.long	.LASF128
	.uleb128 0x4
	.long	0x4f
	.uleb128 0x4
	.long	0x83
	.uleb128 0xe
	.long	0x4f
	.long	0x744
	.uleb128 0xf
	.long	0x3a
	.byte	0xf
	.byte	0
	.uleb128 0x4
	.long	0x6fe
	.uleb128 0x31
	.byte	0x7
	.byte	0x4
	.long	0x41
	.byte	0x12
	.byte	0x78
	.byte	0x1
	.long	0x787
	.uleb128 0x1e
	.string	"GET"
	.byte	0
	.uleb128 0x1e
	.string	"PUT"
	.byte	0x1
	.uleb128 0x10
	.long	.LASF129
	.byte	0x2
	.uleb128 0x1e
	.string	"LS"
	.byte	0x3
	.uleb128 0x10
	.long	.LASF130
	.byte	0x4
	.uleb128 0x10
	.long	.LASF131
	.byte	0x5
	.uleb128 0x10
	.long	.LASF132
	.byte	0x6
	.uleb128 0x10
	.long	.LASF133
	.byte	0x7
	.byte	0
	.uleb128 0x5
	.long	.LASF134
	.byte	0x12
	.byte	0x81
	.byte	0x3
	.long	0x749
	.uleb128 0x11
	.long	.LASF135
	.byte	0x28
	.byte	0x12
	.byte	0x86
	.byte	0x10
	.long	0x7e2
	.uleb128 0x2
	.long	.LASF136
	.byte	0x12
	.byte	0x87
	.byte	0x9
	.long	0x83
	.byte	0
	.uleb128 0x2
	.long	.LASF137
	.byte	0x12
	.byte	0x88
	.byte	0xb
	.long	0xcd
	.byte	0x8
	.uleb128 0x2
	.long	.LASF138
	.byte	0x12
	.byte	0x89
	.byte	0xb
	.long	0xcd
	.byte	0x10
	.uleb128 0x2
	.long	.LASF139
	.byte	0x12
	.byte	0x8a
	.byte	0x9
	.long	0x83
	.byte	0x18
	.uleb128 0x2
	.long	.LASF140
	.byte	0x12
	.byte	0x8b
	.byte	0x1d
	.long	0x7e2
	.byte	0x20
	.byte	0
	.uleb128 0x4
	.long	0x793
	.uleb128 0x5
	.long	.LASF135
	.byte	0x12
	.byte	0x8c
	.byte	0x2
	.long	0x793
	.uleb128 0x32
	.byte	0x68
	.byte	0x12
	.byte	0x95
	.byte	0x9
	.long	0x87f
	.uleb128 0x2
	.long	.LASF141
	.byte	0x12
	.byte	0x97
	.byte	0x9
	.long	0x83
	.byte	0
	.uleb128 0x2
	.long	.LASF142
	.byte	0x12
	.byte	0x98
	.byte	0xb
	.long	0xcd
	.byte	0x8
	.uleb128 0x2
	.long	.LASF143
	.byte	0x12
	.byte	0x99
	.byte	0xb
	.long	0xcd
	.byte	0x10
	.uleb128 0x2
	.long	.LASF144
	.byte	0x12
	.byte	0x9a
	.byte	0x10
	.long	0x787
	.byte	0x18
	.uleb128 0x2
	.long	.LASF145
	.byte	0x12
	.byte	0x9b
	.byte	0x16
	.long	0x87f
	.byte	0x20
	.uleb128 0x2
	.long	.LASF146
	.byte	0x12
	.byte	0x9c
	.byte	0x16
	.long	0x6ce
	.byte	0x28
	.uleb128 0x2
	.long	.LASF147
	.byte	0x12
	.byte	0x9d
	.byte	0xa
	.long	0x72f
	.byte	0x30
	.uleb128 0x2
	.long	.LASF148
	.byte	0x12
	.byte	0x9e
	.byte	0x9
	.long	0x83
	.byte	0x38
	.uleb128 0x2
	.long	.LASF149
	.byte	0x12
	.byte	0x9f
	.byte	0x9
	.long	0x83
	.byte	0x3c
	.uleb128 0x2
	.long	.LASF150
	.byte	0x12
	.byte	0xa1
	.byte	0x15
	.long	0x3fe
	.byte	0x40
	.byte	0
	.uleb128 0x4
	.long	0x7e7
	.uleb128 0x5
	.long	.LASF151
	.byte	0x12
	.byte	0xa2
	.byte	0x3
	.long	0x7f3
	.uleb128 0x11
	.long	.LASF152
	.byte	0xc
	.byte	0x1
	.byte	0x2d
	.byte	0x10
	.long	0x8d2
	.uleb128 0x2
	.long	.LASF142
	.byte	0x1
	.byte	0x2f
	.byte	0xd
	.long	0x415
	.byte	0
	.uleb128 0x2
	.long	.LASF153
	.byte	0x1
	.byte	0x30
	.byte	0xd
	.long	0x415
	.byte	0x1
	.uleb128 0x2
	.long	.LASF154
	.byte	0x1
	.byte	0x31
	.byte	0xe
	.long	0x42d
	.byte	0x4
	.uleb128 0x2
	.long	.LASF155
	.byte	0x1
	.byte	0x32
	.byte	0xe
	.long	0x42d
	.byte	0x8
	.byte	0
	.uleb128 0x5
	.long	.LASF156
	.byte	0x1
	.byte	0x33
	.byte	0x3
	.long	0x890
	.uleb128 0x1f
	.long	.LASF177
	.byte	0x14
	.value	0x23d
	.byte	0xc
	.long	0x83
	.uleb128 0xc
	.long	.LASF157
	.byte	0x13
	.byte	0xbb
	.byte	0xe
	.long	0xcd
	.long	0x901
	.uleb128 0x1
	.long	0x2d7
	.byte	0
	.uleb128 0xc
	.long	.LASF158
	.byte	0x13
	.byte	0x9c
	.byte	0xc
	.long	0x83
	.long	0x91c
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x1
	.long	0x2d7
	.byte	0
	.uleb128 0xc
	.long	.LASF159
	.byte	0x13
	.byte	0x90
	.byte	0xe
	.long	0xcd
	.long	0x93c
	.uleb128 0x1
	.long	0xcd
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x1
	.long	0x3a
	.byte	0
	.uleb128 0xa
	.long	.LASF160
	.byte	0x13
	.value	0x111
	.byte	0xe
	.long	0xcd
	.long	0x958
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xc
	.long	.LASF161
	.byte	0x13
	.byte	0xf6
	.byte	0xe
	.long	0xcd
	.long	0x973
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xc
	.long	.LASF162
	.byte	0x6
	.byte	0xb8
	.byte	0xc
	.long	0x83
	.long	0x989
	.uleb128 0x1
	.long	0x2ed
	.byte	0
	.uleb128 0xa
	.long	.LASF163
	.byte	0x6
	.value	0x2df
	.byte	0xf
	.long	0x2e
	.long	0x9af
	.uleb128 0x1
	.long	0x40a
	.uleb128 0x1
	.long	0x3a
	.uleb128 0x1
	.long	0x3a
	.uleb128 0x1
	.long	0x2ed
	.byte	0
	.uleb128 0xa
	.long	.LASF164
	.byte	0x6
	.value	0x193
	.byte	0xc
	.long	0x83
	.long	0x9cc
	.uleb128 0x1
	.long	0x44b
	.uleb128 0x1
	.long	0x2dc
	.uleb128 0x20
	.byte	0
	.uleb128 0xc
	.long	.LASF165
	.byte	0x13
	.byte	0x2b
	.byte	0xe
	.long	0x48
	.long	0x9ec
	.uleb128 0x1
	.long	0x48
	.uleb128 0x1
	.long	0x40a
	.uleb128 0x1
	.long	0x3a
	.byte	0
	.uleb128 0xa
	.long	.LASF166
	.byte	0x6
	.value	0x2d8
	.byte	0xf
	.long	0x2e
	.long	0xa12
	.uleb128 0x1
	.long	0x4a
	.uleb128 0x1
	.long	0x2e
	.uleb128 0x1
	.long	0x2e
	.uleb128 0x1
	.long	0x2f2
	.byte	0
	.uleb128 0xa
	.long	.LASF167
	.byte	0x6
	.value	0x307
	.byte	0x11
	.long	0x96
	.long	0xa29
	.uleb128 0x1
	.long	0x2ed
	.byte	0
	.uleb128 0xa
	.long	.LASF168
	.byte	0x6
	.value	0x301
	.byte	0xc
	.long	0x83
	.long	0xa4a
	.uleb128 0x1
	.long	0x2ed
	.uleb128 0x1
	.long	0x96
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xa
	.long	.LASF169
	.byte	0x6
	.value	0x108
	.byte	0xe
	.long	0x2ed
	.long	0xa66
	.uleb128 0x1
	.long	0x2dc
	.uleb128 0x1
	.long	0x2dc
	.byte	0
	.uleb128 0xc
	.long	.LASF170
	.byte	0x13
	.byte	0x9f
	.byte	0xc
	.long	0x83
	.long	0xa86
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x1
	.long	0x3a
	.byte	0
	.uleb128 0xc
	.long	.LASF171
	.byte	0xf
	.byte	0x91
	.byte	0x10
	.long	0x2e1
	.long	0xaab
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x48
	.uleb128 0x1
	.long	0x2e
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xc
	.long	.LASF172
	.byte	0xf
	.byte	0x8a
	.byte	0x10
	.long	0x2e1
	.long	0xad0
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x40a
	.uleb128 0x1
	.long	0x2e
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xa
	.long	.LASF173
	.byte	0x13
	.value	0x197
	.byte	0xf
	.long	0x2e
	.long	0xae7
	.uleb128 0x1
	.long	0x2d7
	.byte	0
	.uleb128 0xa
	.long	.LASF174
	.byte	0x14
	.value	0x2a0
	.byte	0xe
	.long	0x48
	.long	0xafe
	.uleb128 0x1
	.long	0x3a
	.byte	0
	.uleb128 0x21
	.long	.LASF189
	.byte	0x10
	.value	0x29a
	.byte	0xd
	.long	0xb11
	.uleb128 0x1
	.long	0x6ce
	.byte	0
	.uleb128 0xa
	.long	.LASF175
	.byte	0x6
	.value	0x16b
	.byte	0xc
	.long	0x83
	.long	0xb29
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x20
	.byte	0
	.uleb128 0xc
	.long	.LASF176
	.byte	0x15
	.byte	0x40
	.byte	0x14
	.long	0x2d7
	.long	0xb4e
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x40f
	.uleb128 0x1
	.long	0xd2
	.uleb128 0x1
	.long	0x439
	.byte	0
	.uleb128 0x26
	.long	.LASF178
	.byte	0x16
	.byte	0x22
	.byte	0x10
	.long	0xb5
	.uleb128 0xa
	.long	.LASF179
	.byte	0x13
	.value	0x1a3
	.byte	0xe
	.long	0xcd
	.long	0xb71
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0x26
	.long	.LASF180
	.byte	0x17
	.byte	0x25
	.byte	0xd
	.long	0x72f
	.uleb128 0xc
	.long	.LASF181
	.byte	0xf
	.byte	0x7e
	.byte	0xc
	.long	0x83
	.long	0xb9d
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x57d
	.uleb128 0x1
	.long	0x439
	.byte	0
	.uleb128 0xc
	.long	.LASF182
	.byte	0xf
	.byte	0x66
	.byte	0xc
	.long	0x83
	.long	0xbbd
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xa
	.long	.LASF183
	.byte	0x6
	.value	0x165
	.byte	0xc
	.long	0x83
	.long	0xbda
	.uleb128 0x1
	.long	0x2ed
	.uleb128 0x1
	.long	0x2d7
	.uleb128 0x20
	.byte	0
	.uleb128 0xa
	.long	.LASF184
	.byte	0x10
	.value	0x29d
	.byte	0x14
	.long	0x2d7
	.long	0xbf1
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xa
	.long	.LASF185
	.byte	0x10
	.value	0x294
	.byte	0xc
	.long	0x83
	.long	0xc17
	.uleb128 0x1
	.long	0x2dc
	.uleb128 0x1
	.long	0x2dc
	.uleb128 0x1
	.long	0x6d8
	.uleb128 0x1
	.long	0xc1c
	.byte	0
	.uleb128 0x4
	.long	0x6ce
	.uleb128 0x12
	.long	0xc17
	.uleb128 0x33
	.long	.LASF186
	.byte	0x14
	.value	0x2f4
	.byte	0xd
	.long	0xc34
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0xc
	.long	.LASF187
	.byte	0x14
	.byte	0x69
	.byte	0xc
	.long	0x83
	.long	0xc4a
	.uleb128 0x1
	.long	0x2d7
	.byte	0
	.uleb128 0xc
	.long	.LASF188
	.byte	0x13
	.byte	0x3d
	.byte	0xe
	.long	0x48
	.long	0xc6a
	.uleb128 0x1
	.long	0x48
	.uleb128 0x1
	.long	0x83
	.uleb128 0x1
	.long	0x3a
	.byte	0
	.uleb128 0x21
	.long	.LASF190
	.byte	0x14
	.value	0x2af
	.byte	0xd
	.long	0xc7d
	.uleb128 0x1
	.long	0x48
	.byte	0
	.uleb128 0xa
	.long	.LASF191
	.byte	0xb
	.value	0x166
	.byte	0xc
	.long	0x83
	.long	0xc94
	.uleb128 0x1
	.long	0x83
	.byte	0
	.uleb128 0x21
	.long	.LASF192
	.byte	0x18
	.value	0x2c0
	.byte	0x6
	.long	0xca7
	.uleb128 0x1
	.long	0xca7
	.byte	0
	.uleb128 0x4
	.long	0x708
	.uleb128 0xa
	.long	.LASF193
	.byte	0x18
	.value	0x2cf
	.byte	0xc
	.long	0x83
	.long	0xccd
	.uleb128 0x1
	.long	0xca7
	.uleb128 0x1
	.long	0x72a
	.uleb128 0x1
	.long	0xccd
	.byte	0
	.uleb128 0x4
	.long	0x41
	.uleb128 0xa
	.long	.LASF194
	.byte	0x18
	.value	0x2cd
	.byte	0xc
	.long	0x83
	.long	0xcf3
	.uleb128 0x1
	.long	0xca7
	.uleb128 0x1
	.long	0x40a
	.uleb128 0x1
	.long	0x2e
	.byte	0
	.uleb128 0xa
	.long	.LASF195
	.byte	0x18
	.value	0x2cb
	.byte	0xc
	.long	0x83
	.long	0xd14
	.uleb128 0x1
	.long	0xca7
	.uleb128 0x1
	.long	0x744
	.uleb128 0x1
	.long	0xd14
	.byte	0
	.uleb128 0x4
	.long	0x719
	.uleb128 0x1f
	.long	.LASF196
	.byte	0x18
	.value	0x391
	.byte	0xf
	.long	0x744
	.uleb128 0x1f
	.long	.LASF197
	.byte	0x18
	.value	0x2be
	.byte	0xd
	.long	0xca7
	.uleb128 0x22
	.long	.LASF219
	.value	0x3f1
	.byte	0x7
	.long	0x48
	.quad	.LFB333
	.quad	.LFE333-.LFB333
	.uleb128 0x1
	.byte	0x9c
	.long	0xd64
	.uleb128 0x16
	.string	"sd"
	.value	0x3f1
	.byte	0x21
	.long	0xd64
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.uleb128 0x4
	.long	0x884
	.uleb128 0x18
	.long	.LASF200
	.value	0x37e
	.quad	.LFB332
	.quad	.LFE332-.LFB332
	.uleb128 0x1
	.byte	0x9c
	.long	0xdc0
	.uleb128 0x16
	.string	"sd"
	.value	0x37e
	.byte	0x21
	.long	0xd64
	.uleb128 0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x8
	.string	"i"
	.value	0x384
	.byte	0x9
	.long	0x83
	.uleb128 0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x3
	.long	.LASF198
	.value	0x385
	.byte	0x9
	.long	0x83
	.uleb128 0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x3
	.long	.LASF199
	.value	0x386
	.byte	0x16
	.long	0x87f
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.uleb128 0x18
	.long	.LASF201
	.value	0x34b
	.quad	.LFB331
	.quad	.LFE331-.LFB331
	.uleb128 0x1
	.byte	0x9c
	.long	0xf29
	.uleb128 0x16
	.string	"sd"
	.value	0x34b
	.byte	0x21
	.long	0xd64
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30840
	.uleb128 0x8
	.string	"i"
	.value	0x34e
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30824
	.uleb128 0x3
	.long	.LASF202
	.value	0x34e
	.byte	0x10
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30812
	.uleb128 0x3
	.long	.LASF199
	.value	0x34f
	.byte	0x16
	.long	0x87f
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30792
	.uleb128 0x3
	.long	.LASF203
	.value	0x351
	.byte	0xa
	.long	0xf29
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30768
	.uleb128 0x19
	.long	.LASF140
	.value	0x377
	.byte	0x5
	.quad	.L173
	.uleb128 0x7
	.quad	.LBB39
	.quad	.LBE39-.LBB39
	.uleb128 0x3
	.long	.LASF204
	.value	0x358
	.byte	0xd
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30808
	.uleb128 0x3
	.long	.LASF205
	.value	0x359
	.byte	0xd
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30820
	.uleb128 0x7
	.quad	.LBB40
	.quad	.LBE40-.LBB40
	.uleb128 0x8
	.string	"j"
	.value	0x35a
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30816
	.uleb128 0x7
	.quad	.LBB41
	.quad	.LBE41-.LBB41
	.uleb128 0x3
	.long	.LASF206
	.value	0x35e
	.byte	0x1e
	.long	0x8d2
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30780
	.uleb128 0x6
	.quad	.LBB42
	.quad	.LBE42-.LBB42
	.long	0xedb
	.uleb128 0x3
	.long	.LASF202
	.value	0x364
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30804
	.byte	0
	.uleb128 0x6
	.quad	.LBB43
	.quad	.LBE43-.LBB43
	.long	0xf02
	.uleb128 0x3
	.long	.LASF202
	.value	0x365
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30800
	.byte	0
	.uleb128 0x7
	.quad	.LBB44
	.quad	.LBE44-.LBB44
	.uleb128 0x3
	.long	.LASF202
	.value	0x367
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30796
	.byte	0
	.byte	0
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0xe
	.long	0xd7
	.long	0xf3a
	.uleb128 0x34
	.long	0x3a
	.value	0x77ff
	.byte	0
	.uleb128 0x18
	.long	.LASF207
	.value	0x28f
	.quad	.LFB330
	.quad	.LFE330-.LFB330
	.uleb128 0x1
	.byte	0x9c
	.long	0x1158
	.uleb128 0x16
	.string	"sd"
	.value	0x28f
	.byte	0x1f
	.long	0xd64
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30872
	.uleb128 0x8
	.string	"i"
	.value	0x293
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30848
	.uleb128 0x3
	.long	.LASF202
	.value	0x293
	.byte	0x10
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30820
	.uleb128 0x3
	.long	.LASF199
	.value	0x294
	.byte	0x16
	.long	0x87f
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30808
	.uleb128 0x3
	.long	.LASF203
	.value	0x296
	.byte	0xa
	.long	0xf29
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30752
	.uleb128 0x1d
	.long	.LASF208
	.byte	0x20
	.byte	0x1
	.value	0x299
	.byte	0x14
	.long	0xfe2
	.uleb128 0xb
	.long	.LASF143
	.byte	0x1
	.value	0x29a
	.byte	0xf
	.long	0xcd
	.byte	0
	.uleb128 0xb
	.long	.LASF209
	.byte	0x1
	.value	0x29b
	.byte	0xd
	.long	0x1158
	.byte	0x8
	.uleb128 0xb
	.long	.LASF140
	.byte	0x1
	.value	0x29c
	.byte	0x1b
	.long	0x1168
	.byte	0x18
	.byte	0
	.uleb128 0x25
	.long	.LASF210
	.byte	0x1
	.value	0x29d
	.byte	0x7
	.long	0xfa9
	.uleb128 0x3
	.long	.LASF211
	.value	0x29f
	.byte	0x12
	.long	0x1000
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30800
	.uleb128 0x4
	.long	0xfe2
	.uleb128 0x19
	.long	.LASF140
	.value	0x2f7
	.byte	0x5
	.quad	.L138
	.uleb128 0x6
	.quad	.LBB31
	.quad	.LBE31-.LBB31
	.long	0x10a9
	.uleb128 0x3
	.long	.LASF206
	.value	0x2aa
	.byte	0x1a
	.long	0x8d2
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30764
	.uleb128 0x3
	.long	.LASF212
	.value	0x2b3
	.byte	0xd
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30844
	.uleb128 0x35
	.long	.LLRL0
	.uleb128 0x3
	.long	.LASF213
	.value	0x2c7
	.byte	0x13
	.long	0xcd
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30816
	.uleb128 0x3
	.long	.LASF153
	.value	0x2c8
	.byte	0x11
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30852
	.uleb128 0x7
	.quad	.LBB33
	.quad	.LBE33-.LBB33
	.uleb128 0x3
	.long	.LASF214
	.value	0x2ce
	.byte	0x1e
	.long	0x1000
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30792
	.uleb128 0x3
	.long	.LASF215
	.value	0x2cf
	.byte	0x15
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30840
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0x7
	.quad	.LBB35
	.quad	.LBE35-.LBB35
	.uleb128 0x3
	.long	.LASF214
	.value	0x30c
	.byte	0x16
	.long	0x1000
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30784
	.uleb128 0x7
	.quad	.LBB36
	.quad	.LBE36-.LBB36
	.uleb128 0x3
	.long	.LASF216
	.value	0x310
	.byte	0x11
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30836
	.uleb128 0x3
	.long	.LASF217
	.value	0x311
	.byte	0x11
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30832
	.uleb128 0x3
	.long	.LASF218
	.value	0x33d
	.byte	0x1a
	.long	0x1000
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30776
	.uleb128 0x6
	.quad	.LBB37
	.quad	.LBE37-.LBB37
	.long	0x1134
	.uleb128 0x8
	.string	"j"
	.value	0x313
	.byte	0x16
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30828
	.byte	0
	.uleb128 0x7
	.quad	.LBB38
	.quad	.LBE38-.LBB38
	.uleb128 0x8
	.string	"j"
	.value	0x32c
	.byte	0x16
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30824
	.byte	0
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0xe
	.long	0x83
	.long	0x1168
	.uleb128 0xf
	.long	0x3a
	.byte	0x3
	.byte	0
	.uleb128 0x4
	.long	0xfa9
	.uleb128 0x22
	.long	.LASF220
	.value	0x267
	.byte	0x5
	.long	0x83
	.quad	.LFB329
	.quad	.LFE329-.LFB329
	.uleb128 0x1
	.byte	0x9c
	.long	0x11eb
	.uleb128 0x1b
	.long	.LASF221
	.value	0x267
	.byte	0x27
	.long	0x2d7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -56
	.uleb128 0x1b
	.long	.LASF213
	.value	0x267
	.byte	0x3f
	.long	0x446
	.uleb128 0x2
	.byte	0x91
	.sleb128 -64
	.uleb128 0x1b
	.long	.LASF153
	.value	0x267
	.byte	0x57
	.long	0x72f
	.uleb128 0x3
	.byte	0x91
	.sleb128 -72
	.uleb128 0x3
	.long	.LASF222
	.value	0x26a
	.byte	0x11
	.long	0x2d7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x3
	.long	.LASF223
	.value	0x275
	.byte	0x11
	.long	0x2d7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x3
	.long	.LASF224
	.value	0x27f
	.byte	0xc
	.long	0x2e
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.uleb128 0x22
	.long	.LASF225
	.value	0x242
	.byte	0x7
	.long	0xcd
	.quad	.LFB328
	.quad	.LFE328-.LFB328
	.uleb128 0x1
	.byte	0x9c
	.long	0x1259
	.uleb128 0x1b
	.long	.LASF221
	.value	0x242
	.byte	0x2d
	.long	0x2d7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -56
	.uleb128 0x3
	.long	.LASF226
	.value	0x245
	.byte	0x11
	.long	0x2d7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -48
	.uleb128 0x3
	.long	.LASF223
	.value	0x24f
	.byte	0x11
	.long	0x2d7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x3
	.long	.LASF224
	.value	0x256
	.byte	0xc
	.long	0x2e
	.uleb128 0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x3
	.long	.LASF213
	.value	0x259
	.byte	0xb
	.long	0xcd
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.uleb128 0x18
	.long	.LASF227
	.value	0x14f
	.quad	.LFB327
	.quad	.LFE327-.LFB327
	.uleb128 0x1
	.byte	0x9c
	.long	0x1626
	.uleb128 0x16
	.string	"sd"
	.value	0x14f
	.byte	0x1e
	.long	0xd64
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30984
	.uleb128 0x3
	.long	.LASF204
	.value	0x154
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30908
	.uleb128 0x8
	.string	"i"
	.value	0x156
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30972
	.uleb128 0x3
	.long	.LASF202
	.value	0x156
	.byte	0x10
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30904
	.uleb128 0x3
	.long	.LASF199
	.value	0x157
	.byte	0x16
	.long	0x87f
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30872
	.uleb128 0x3
	.long	.LASF228
	.value	0x15b
	.byte	0x9
	.long	0x1158
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30832
	.uleb128 0x3
	.long	.LASF229
	.value	0x15c
	.byte	0x9
	.long	0x1158
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30816
	.uleb128 0x3
	.long	.LASF230
	.value	0x15d
	.byte	0xb
	.long	0x1626
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30800
	.uleb128 0x3
	.long	.LASF203
	.value	0x15e
	.byte	0xa
	.long	0xf29
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30768
	.uleb128 0x3
	.long	.LASF231
	.value	0x15f
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30968
	.uleb128 0x3
	.long	.LASF232
	.value	0x167
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30964
	.uleb128 0x3
	.long	.LASF233
	.value	0x168
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30960
	.uleb128 0x19
	.long	.LASF140
	.value	0x1d9
	.byte	0x5
	.quad	.L68
	.uleb128 0x19
	.long	.LASF234
	.value	0x1d0
	.byte	0x9
	.quad	.L83
	.uleb128 0x3
	.long	.LASF217
	.value	0x1f4
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30936
	.uleb128 0x3
	.long	.LASF143
	.value	0x213
	.byte	0xb
	.long	0xcd
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30848
	.uleb128 0x8
	.string	"fs"
	.value	0x219
	.byte	0xb
	.long	0x2ed
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30856
	.uleb128 0x3
	.long	.LASF235
	.value	0x228
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30920
	.uleb128 0x6
	.quad	.LBB15
	.quad	.LBE15-.LBB15
	.long	0x154b
	.uleb128 0x3
	.long	.LASF205
	.value	0x178
	.byte	0xd
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30956
	.uleb128 0x6
	.quad	.LBB16
	.quad	.LBE16-.LBB16
	.long	0x13ed
	.uleb128 0x8
	.string	"j"
	.value	0x17b
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30952
	.byte	0
	.uleb128 0x7
	.quad	.LBB17
	.quad	.LBE17-.LBB17
	.uleb128 0x8
	.string	"j"
	.value	0x181
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30948
	.uleb128 0x7
	.quad	.LBB18
	.quad	.LBE18-.LBB18
	.uleb128 0x3
	.long	.LASF206
	.value	0x18e
	.byte	0x1e
	.long	0x8d2
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30848
	.uleb128 0x3
	.long	.LASF236
	.value	0x1a5
	.byte	0x1f
	.long	0x1636
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30864
	.uleb128 0x3
	.long	.LASF237
	.value	0x1a8
	.byte	0x11
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30884
	.uleb128 0x3
	.long	.LASF238
	.value	0x1b4
	.byte	0x11
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30944
	.uleb128 0x6
	.quad	.LBB19
	.quad	.LBE19-.LBB19
	.long	0x1489
	.uleb128 0x3
	.long	.LASF202
	.value	0x196
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30900
	.byte	0
	.uleb128 0x6
	.quad	.LBB20
	.quad	.LBE20-.LBB20
	.long	0x14b0
	.uleb128 0x3
	.long	.LASF202
	.value	0x197
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30896
	.byte	0
	.uleb128 0x6
	.quad	.LBB21
	.quad	.LBE21-.LBB21
	.long	0x14d7
	.uleb128 0x3
	.long	.LASF202
	.value	0x19a
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30892
	.byte	0
	.uleb128 0x6
	.quad	.LBB22
	.quad	.LBE22-.LBB22
	.long	0x14fe
	.uleb128 0x3
	.long	.LASF202
	.value	0x1a6
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30888
	.byte	0
	.uleb128 0x6
	.quad	.LBB23
	.quad	.LBE23-.LBB23
	.long	0x1525
	.uleb128 0x3
	.long	.LASF202
	.value	0x1b8
	.byte	0x1c
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30876
	.byte	0
	.uleb128 0x7
	.quad	.LBB24
	.quad	.LBE24-.LBB24
	.uleb128 0x3
	.long	.LASF202
	.value	0x1c4
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30880
	.byte	0
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0x6
	.quad	.LBB25
	.quad	.LBE25-.LBB25
	.long	0x1570
	.uleb128 0x8
	.string	"i"
	.value	0x1e9
	.byte	0xe
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30940
	.byte	0
	.uleb128 0x6
	.quad	.LBB26
	.quad	.LBE26-.LBB26
	.long	0x1595
	.uleb128 0x8
	.string	"i"
	.value	0x1f5
	.byte	0xe
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30932
	.byte	0
	.uleb128 0x6
	.quad	.LBB27
	.quad	.LBE27-.LBB27
	.long	0x15ba
	.uleb128 0x8
	.string	"i"
	.value	0x208
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30928
	.byte	0
	.uleb128 0x6
	.quad	.LBB28
	.quad	.LBE28-.LBB28
	.long	0x15df
	.uleb128 0x8
	.string	"i"
	.value	0x21e
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30924
	.byte	0
	.uleb128 0x6
	.quad	.LBB29
	.quad	.LBE29-.LBB29
	.long	0x1604
	.uleb128 0x8
	.string	"i"
	.value	0x229
	.byte	0xe
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30916
	.byte	0
	.uleb128 0x7
	.quad	.LBB30
	.quad	.LBE30-.LBB30
	.uleb128 0x8
	.string	"i"
	.value	0x22e
	.byte	0xe
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30912
	.byte	0
	.byte	0
	.uleb128 0xe
	.long	0xcd
	.long	0x1636
	.uleb128 0xf
	.long	0x3a
	.byte	0x3
	.byte	0
	.uleb128 0x4
	.long	0x8d2
	.uleb128 0x18
	.long	.LASF239
	.value	0x121
	.quad	.LFB326
	.quad	.LFE326-.LFB326
	.uleb128 0x1
	.byte	0x9c
	.long	0x1718
	.uleb128 0x16
	.string	"sd"
	.value	0x121
	.byte	0x1e
	.long	0xd64
	.uleb128 0x3
	.byte	0x91
	.sleb128 -136
	.uleb128 0x8
	.string	"fs"
	.value	0x123
	.byte	0xb
	.long	0x2ed
	.uleb128 0x3
	.byte	0x91
	.sleb128 -96
	.uleb128 0x3
	.long	.LASF204
	.value	0x129
	.byte	0xe
	.long	0x42d
	.uleb128 0x3
	.byte	0x91
	.sleb128 -104
	.uleb128 0x3
	.long	.LASF240
	.value	0x12c
	.byte	0x9
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -100
	.uleb128 0x3
	.long	.LASF241
	.value	0x12f
	.byte	0x9
	.long	0x1158
	.uleb128 0x3
	.byte	0x91
	.sleb128 -80
	.uleb128 0x3
	.long	.LASF230
	.value	0x130
	.byte	0xb
	.long	0x1626
	.uleb128 0x2
	.byte	0x91
	.sleb128 -64
	.uleb128 0x3
	.long	.LASF242
	.value	0x131
	.byte	0x9
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -116
	.uleb128 0x7
	.quad	.LBB13
	.quad	.LBE13-.LBB13
	.uleb128 0x8
	.string	"i"
	.value	0x138
	.byte	0xe
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -112
	.uleb128 0x7
	.quad	.LBB14
	.quad	.LBE14-.LBB14
	.uleb128 0x3
	.long	.LASF243
	.value	0x13a
	.byte	0xd
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -108
	.uleb128 0x3
	.long	.LASF244
	.value	0x143
	.byte	0xf
	.long	0xcd
	.uleb128 0x3
	.byte	0x91
	.sleb128 -88
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0x23
	.long	.LASF245
	.byte	0xc1
	.quad	.LFB325
	.quad	.LFE325-.LFB325
	.uleb128 0x1
	.byte	0x9c
	.long	0x1913
	.uleb128 0x1a
	.string	"sd"
	.byte	0xc1
	.byte	0x2c
	.long	0xd64
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30856
	.uleb128 0x13
	.long	.LASF230
	.byte	0xc1
	.byte	0x36
	.long	0x446
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30864
	.uleb128 0x13
	.long	.LASF241
	.byte	0xc1
	.byte	0x44
	.long	0x72f
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30872
	.uleb128 0x13
	.long	.LASF204
	.byte	0xc1
	.byte	0x57
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30876
	.uleb128 0x9
	.long	.LASF199
	.byte	0xc8
	.byte	0x16
	.long	0x87f
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30808
	.uleb128 0x17
	.string	"i"
	.byte	0xc9
	.byte	0x9
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30848
	.uleb128 0x9
	.long	.LASF202
	.byte	0xc9
	.byte	0x10
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30824
	.uleb128 0x9
	.long	.LASF228
	.byte	0xcc
	.byte	0x9
	.long	0x1158
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30784
	.uleb128 0x9
	.long	.LASF203
	.byte	0xcd
	.byte	0xa
	.long	0xf29
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30768
	.uleb128 0x19
	.long	.LASF140
	.value	0x101
	.byte	0x5
	.quad	.L36
	.uleb128 0x6
	.quad	.LBB4
	.quad	.LBE4-.LBB4
	.long	0x18f1
	.uleb128 0x9
	.long	.LASF205
	.byte	0xd6
	.byte	0xd
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30844
	.uleb128 0x6
	.quad	.LBB5
	.quad	.LBE5-.LBB5
	.long	0x181a
	.uleb128 0x17
	.string	"j"
	.byte	0xda
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30840
	.byte	0
	.uleb128 0x7
	.quad	.LBB6
	.quad	.LBE6-.LBB6
	.uleb128 0x17
	.string	"j"
	.byte	0xe0
	.byte	0x12
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30836
	.uleb128 0x7
	.quad	.LBB7
	.quad	.LBE7-.LBB7
	.uleb128 0x9
	.long	.LASF246
	.byte	0xe6
	.byte	0x1e
	.long	0x8d2
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30796
	.uleb128 0x6
	.quad	.LBB8
	.quad	.LBE8-.LBB8
	.long	0x1880
	.uleb128 0x9
	.long	.LASF202
	.byte	0xec
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30828
	.byte	0
	.uleb128 0x6
	.quad	.LBB9
	.quad	.LBE9-.LBB9
	.long	0x18a6
	.uleb128 0x9
	.long	.LASF202
	.byte	0xee
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30820
	.byte	0
	.uleb128 0x6
	.quad	.LBB10
	.quad	.LBE10-.LBB10
	.long	0x18cc
	.uleb128 0x9
	.long	.LASF202
	.byte	0xf0
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30816
	.byte	0
	.uleb128 0x7
	.quad	.LBB11
	.quad	.LBE11-.LBB11
	.uleb128 0x9
	.long	.LASF202
	.byte	0xf3
	.byte	0x18
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30812
	.byte	0
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0x7
	.quad	.LBB12
	.quad	.LBE12-.LBB12
	.uleb128 0x8
	.string	"i"
	.value	0x107
	.byte	0xe
	.long	0x83
	.uleb128 0x4
	.byte	0x91
	.sleb128 -30832
	.byte	0
	.byte	0
	.uleb128 0x36
	.long	.LASF247
	.byte	0x1
	.byte	0x7b
	.byte	0x5
	.long	0x83
	.quad	.LFB324
	.quad	.LFE324-.LFB324
	.uleb128 0x1
	.byte	0x9c
	.long	0x19ac
	.uleb128 0x1a
	.string	"sd"
	.byte	0x7b
	.byte	0x23
	.long	0xd64
	.uleb128 0x3
	.byte	0x91
	.sleb128 -168
	.uleb128 0x13
	.long	.LASF199
	.byte	0x7b
	.byte	0x38
	.long	0x87f
	.uleb128 0x3
	.byte	0x91
	.sleb128 -176
	.uleb128 0x13
	.long	.LASF248
	.byte	0x7b
	.byte	0x45
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -180
	.uleb128 0x9
	.long	.LASF249
	.byte	0x7d
	.byte	0x15
	.long	0x64a
	.uleb128 0x3
	.byte	0x91
	.sleb128 -144
	.uleb128 0x9
	.long	.LASF218
	.byte	0x7d
	.byte	0x1d
	.long	0x6ce
	.uleb128 0x3
	.byte	0x91
	.sleb128 -152
	.uleb128 0x9
	.long	.LASF250
	.byte	0x7e
	.byte	0x9
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -156
	.uleb128 0x37
	.long	.LASF251
	.byte	0x1
	.byte	0xbc
	.byte	0x1
	.quad	.L23
	.uleb128 0x17
	.string	"s"
	.byte	0xb4
	.byte	0xa
	.long	0x19ac
	.uleb128 0x3
	.byte	0x91
	.sleb128 -96
	.byte	0
	.uleb128 0xe
	.long	0xd7
	.long	0x19bc
	.uleb128 0xf
	.long	0x3a
	.byte	0x2d
	.byte	0
	.uleb128 0x23
	.long	.LASF252
	.byte	0x6b
	.quad	.LFB323
	.quad	.LFE323-.LFB323
	.uleb128 0x1
	.byte	0x9c
	.long	0x1a14
	.uleb128 0x1a
	.string	"sd"
	.byte	0x6b
	.byte	0x28
	.long	0xd64
	.uleb128 0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x9
	.long	.LASF199
	.byte	0x6d
	.byte	0x16
	.long	0x87f
	.uleb128 0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x7
	.quad	.LBB3
	.quad	.LBE3-.LBB3
	.uleb128 0x9
	.long	.LASF218
	.byte	0x70
	.byte	0x1a
	.long	0x87f
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.byte	0
	.uleb128 0x23
	.long	.LASF253
	.byte	0x5f
	.quad	.LFB322
	.quad	.LFE322-.LFB322
	.uleb128 0x1
	.byte	0x9c
	.long	0x1a3f
	.uleb128 0x13
	.long	.LASF199
	.byte	0x5f
	.byte	0x32
	.long	0x87f
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.uleb128 0x38
	.long	.LASF254
	.byte	0x1
	.byte	0x55
	.byte	0x7
	.long	0x48
	.quad	.LFB321
	.quad	.LFE321-.LFB321
	.uleb128 0x1
	.byte	0x9c
	.long	0x1a6f
	.uleb128 0x1a
	.string	"sa"
	.byte	0x55
	.byte	0x24
	.long	0x4e1
	.uleb128 0x2
	.byte	0x91
	.sleb128 -24
	.byte	0
	.uleb128 0x39
	.long	.LASF255
	.byte	0x1
	.byte	0x35
	.byte	0xa
	.long	0x42d
	.quad	.LFB320
	.quad	.LFE320-.LFB320
	.uleb128 0x1
	.byte	0x9c
	.uleb128 0x1a
	.string	"str"
	.byte	0x35
	.byte	0x18
	.long	0xcd
	.uleb128 0x3
	.byte	0x91
	.sleb128 -88
	.uleb128 0x13
	.long	.LASF256
	.byte	0x35
	.byte	0x21
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -92
	.uleb128 0x9
	.long	.LASF257
	.byte	0x37
	.byte	0x11
	.long	0xca7
	.uleb128 0x2
	.byte	0x91
	.sleb128 -64
	.uleb128 0x17
	.string	"md"
	.byte	0x38
	.byte	0x13
	.long	0x744
	.uleb128 0x2
	.byte	0x91
	.sleb128 -56
	.uleb128 0x9
	.long	.LASF258
	.byte	0x3a
	.byte	0x12
	.long	0x41
	.uleb128 0x3
	.byte	0x91
	.sleb128 -76
	.uleb128 0x9
	.long	.LASF259
	.byte	0x3b
	.byte	0x13
	.long	0x734
	.uleb128 0x2
	.byte	0x91
	.sleb128 -48
	.uleb128 0x9
	.long	.LASF204
	.byte	0x4c
	.byte	0xe
	.long	0x42d
	.uleb128 0x3
	.byte	0x91
	.sleb128 -72
	.uleb128 0x7
	.quad	.LBB2
	.quad	.LBE2-.LBB2
	.uleb128 0x17
	.string	"n"
	.byte	0x4d
	.byte	0xe
	.long	0x83
	.uleb128 0x3
	.byte	0x91
	.sleb128 -68
	.byte	0
	.byte	0
	.byte	0
	.section	.debug_abbrev,"",@progbits
.Ldebug_abbrev0:
	.uleb128 0x1
	.uleb128 0x5
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x2
	.uleb128 0xd
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x3
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x4
	.uleb128 0xf
	.byte	0
	.uleb128 0xb
	.uleb128 0x21
	.sleb128 8
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x5
	.uleb128 0x16
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x6
	.uleb128 0xb
	.byte	0x1
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x7
	.uleb128 0xb
	.byte	0x1
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.byte	0
	.byte	0
	.uleb128 0x8
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x9
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0xa
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0xb
	.uleb128 0xd
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0xc
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0xd
	.uleb128 0x24
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3e
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0xe
	.byte	0
	.byte	0
	.uleb128 0xe
	.uleb128 0x1
	.byte	0x1
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0xf
	.uleb128 0x21
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2f
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x10
	.uleb128 0x28
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x1c
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x11
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x12
	.uleb128 0x37
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x13
	.uleb128 0x5
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x14
	.uleb128 0x13
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3c
	.uleb128 0x19
	.byte	0
	.byte	0
	.uleb128 0x15
	.uleb128 0xd
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x16
	.uleb128 0x5
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x17
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x18
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0x21
	.sleb128 6
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x40
	.uleb128 0x18
	.uleb128 0x7c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x19
	.uleb128 0xa
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x11
	.uleb128 0x1
	.byte	0
	.byte	0
	.uleb128 0x1a
	.uleb128 0x5
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x1b
	.uleb128 0x5
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0x18
	.byte	0
	.byte	0
	.uleb128 0x1c
	.uleb128 0x26
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x1d
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x1e
	.uleb128 0x28
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x1c
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x1f
	.uleb128 0x2e
	.byte	0
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3c
	.uleb128 0x19
	.byte	0
	.byte	0
	.uleb128 0x20
	.uleb128 0x18
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0x21
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x3c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x22
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x40
	.uleb128 0x18
	.uleb128 0x7c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x23
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0x21
	.sleb128 1
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0x21
	.sleb128 6
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x40
	.uleb128 0x18
	.uleb128 0x7c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x24
	.uleb128 0x17
	.byte	0x1
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x25
	.uleb128 0x16
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x26
	.uleb128 0x2e
	.byte	0
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3c
	.uleb128 0x19
	.byte	0
	.byte	0
	.uleb128 0x27
	.uleb128 0x11
	.byte	0x1
	.uleb128 0x25
	.uleb128 0xe
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0x1f
	.uleb128 0x1b
	.uleb128 0x1f
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x10
	.uleb128 0x17
	.byte	0
	.byte	0
	.uleb128 0x28
	.uleb128 0xf
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x29
	.uleb128 0x24
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3e
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0x8
	.byte	0
	.byte	0
	.uleb128 0x2a
	.uleb128 0x16
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x2b
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3c
	.uleb128 0x19
	.byte	0
	.byte	0
	.uleb128 0x2c
	.uleb128 0x26
	.byte	0
	.byte	0
	.byte	0
	.uleb128 0x2d
	.uleb128 0x4
	.byte	0x1
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3e
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x2e
	.uleb128 0x28
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x1c
	.uleb128 0x6
	.byte	0
	.byte	0
	.uleb128 0x2f
	.uleb128 0x28
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x1c
	.uleb128 0x5
	.byte	0
	.byte	0
	.uleb128 0x30
	.uleb128 0x17
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x31
	.uleb128 0x4
	.byte	0x1
	.uleb128 0x3e
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x32
	.uleb128 0x13
	.byte	0x1
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x33
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x87
	.uleb128 0x19
	.uleb128 0x3c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x34
	.uleb128 0x21
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2f
	.uleb128 0x5
	.byte	0
	.byte	0
	.uleb128 0x35
	.uleb128 0xb
	.byte	0x1
	.uleb128 0x55
	.uleb128 0x17
	.byte	0
	.byte	0
	.uleb128 0x36
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x40
	.uleb128 0x18
	.uleb128 0x7c
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x37
	.uleb128 0xa
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x11
	.uleb128 0x1
	.byte	0
	.byte	0
	.uleb128 0x38
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x40
	.uleb128 0x18
	.uleb128 0x7a
	.uleb128 0x19
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x39
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0x19
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x39
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0x19
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x7
	.uleb128 0x40
	.uleb128 0x18
	.uleb128 0x7c
	.uleb128 0x19
	.byte	0
	.byte	0
	.byte	0
	.section	.debug_aranges,"",@progbits
	.long	0x2c
	.value	0x2
	.long	.Ldebug_info0
	.byte	0x8
	.byte	0
	.value	0
	.value	0
	.quad	.Ltext0
	.quad	.Letext0-.Ltext0
	.quad	0
	.quad	0
	.section	.debug_rnglists,"",@progbits
.Ldebug_ranges0:
	.long	.Ldebug_ranges3-.Ldebug_ranges2
.Ldebug_ranges2:
	.value	0x5
	.byte	0x8
	.byte	0
	.long	0
.LLRL0:
	.byte	0x4
	.uleb128 .LBB32-.Ltext0
	.uleb128 .LBE32-.Ltext0
	.byte	0x4
	.uleb128 .LBB34-.Ltext0
	.uleb128 .LBE34-.Ltext0
	.byte	0
.Ldebug_ranges3:
	.section	.debug_line,"",@progbits
.Ldebug_line0:
	.section	.debug_str,"MS",@progbits,1
.LASF98:
	.string	"sin6_flowinfo"
.LASF38:
	.string	"_shortbuf"
.LASF177:
	.string	"rand"
.LASF160:
	.string	"strrchr"
.LASF45:
	.string	"_prevchain"
.LASF236:
	.string	"recv_message_header"
.LASF261:
	.string	"_IO_lock_t"
.LASF258:
	.string	"md_len"
.LASF173:
	.string	"strlen"
.LASF254:
	.string	"get_in_addr"
.LASF202:
	.string	"numbytes"
.LASF170:
	.string	"strncmp"
.LASF27:
	.string	"_IO_buf_end"
.LASF246:
	.string	"message"
.LASF87:
	.string	"sockaddr"
.LASF152:
	.string	"message_header_s"
.LASF156:
	.string	"message_header_t"
.LASF100:
	.string	"sin6_scope_id"
.LASF25:
	.string	"_IO_write_end"
.LASF3:
	.string	"unsigned int"
.LASF140:
	.string	"next"
.LASF43:
	.string	"_freeres_list"
.LASF19:
	.string	"_flags"
.LASF234:
	.string	"chunk_failed"
.LASF263:
	.string	"__socket_type"
.LASF222:
	.string	"filename_part"
.LASF179:
	.string	"strerror"
.LASF206:
	.string	"message_header"
.LASF31:
	.string	"_markers"
.LASF185:
	.string	"getaddrinfo"
.LASF228:
	.string	"chunks_stored"
.LASF143:
	.string	"filename"
.LASF247:
	.string	"connect_server"
.LASF127:
	.string	"ENGINE"
.LASF130:
	.string	"EXIT"
.LASF243:
	.string	"chunk_size"
.LASF147:
	.string	"server_sock_fds"
.LASF215:
	.string	"found"
.LASF225:
	.string	"extract_original_filename"
.LASF235:
	.string	"total_file_size"
.LASF136:
	.string	"client_sock_fd"
.LASF244:
	.string	"chunk"
.LASF172:
	.string	"send"
.LASF233:
	.string	"servers_available"
.LASF56:
	.string	"__pthread_internal_list"
.LASF75:
	.string	"uint32_t"
.LASF57:
	.string	"__prev"
.LASF102:
	.string	"in_addr_t"
.LASF259:
	.string	"digest"
.LASF30:
	.string	"_IO_save_end"
.LASF62:
	.string	"__count"
.LASF123:
	.string	"EVP_MD"
.LASF239:
	.string	"put_file"
.LASF197:
	.string	"EVP_MD_CTX_new"
.LASF128:
	.string	"engine_st"
.LASF174:
	.string	"malloc"
.LASF111:
	.string	"addrinfo"
.LASF205:
	.string	"index"
.LASF54:
	.string	"long long unsigned int"
.LASF155:
	.string	"data_length"
.LASF213:
	.string	"original_filename"
.LASF153:
	.string	"chunk_id"
.LASF106:
	.string	"__u6_addr8"
.LASF159:
	.string	"strncpy"
.LASF251:
	.string	"cleanup"
.LASF201:
	.string	"delete_file"
.LASF91:
	.string	"sin_family"
.LASF10:
	.string	"__uint16_t"
.LASF198:
	.string	"servers_online"
.LASF63:
	.string	"__owner"
.LASF40:
	.string	"_offset"
.LASF68:
	.string	"__list"
.LASF105:
	.string	"in_port_t"
.LASF67:
	.string	"__elision"
.LASF183:
	.string	"fprintf"
.LASF33:
	.string	"_fileno"
.LASF97:
	.string	"sin6_port"
.LASF184:
	.string	"gai_strerror"
.LASF94:
	.string	"sin_zero"
.LASF104:
	.string	"s_addr"
.LASF237:
	.string	"data_size"
.LASF7:
	.string	"size_t"
.LASF86:
	.string	"sa_family_t"
.LASF37:
	.string	"_vtable_offset"
.LASF117:
	.string	"ai_addr"
.LASF22:
	.string	"_IO_read_base"
.LASF211:
	.string	"file_list"
.LASF99:
	.string	"sin6_addr"
.LASF190:
	.string	"free"
.LASF158:
	.string	"strcmp"
.LASF210:
	.string	"file_info_t"
.LASF249:
	.string	"hints"
.LASF131:
	.string	"SERVER_INFO"
.LASF58:
	.string	"__next"
.LASF17:
	.string	"char"
.LASF145:
	.string	"servers_details"
.LASF46:
	.string	"_mode"
.LASF242:
	.string	"total_chunk_size_until_now"
.LASF49:
	.string	"_IO_marker"
.LASF191:
	.string	"close"
.LASF180:
	.string	"__errno_location"
.LASF176:
	.string	"inet_ntop"
.LASF186:
	.string	"exit"
.LASF192:
	.string	"EVP_MD_CTX_free"
.LASF82:
	.string	"SOCK_DCCP"
.LASF66:
	.string	"__spins"
.LASF144:
	.string	"command_int"
.LASF230:
	.string	"chunks"
.LASF73:
	.string	"uint8_t"
.LASF250:
	.string	"status"
.LASF83:
	.string	"SOCK_PACKET"
.LASF262:
	.string	"stderr"
.LASF229:
	.string	"chunks_stored_sizes"
.LASF194:
	.string	"EVP_DigestUpdate"
.LASF126:
	.string	"evp_md_ctx_st"
.LASF23:
	.string	"_IO_write_base"
.LASF78:
	.string	"SOCK_DGRAM"
.LASF15:
	.string	"__pid_t"
.LASF135:
	.string	"serverDetails_t"
.LASF53:
	.string	"long long int"
.LASF175:
	.string	"printf"
.LASF150:
	.string	"lock"
.LASF28:
	.string	"_IO_save_base"
.LASF92:
	.string	"sin_port"
.LASF129:
	.string	"DELETE"
.LASF223:
	.string	"last_underscore"
.LASF257:
	.string	"context"
.LASF214:
	.string	"file_entry"
.LASF108:
	.string	"__u6_addr32"
.LASF119:
	.string	"ai_next"
.LASF193:
	.string	"EVP_DigestFinal_ex"
.LASF132:
	.string	"HELP"
.LASF168:
	.string	"fseek"
.LASF188:
	.string	"memset"
.LASF151:
	.string	"sockDetails_t"
.LASF212:
	.string	"files_found"
.LASF245:
	.string	"connect_and_put_chunks"
.LASF240:
	.string	"size"
.LASF44:
	.string	"_freeres_buf"
.LASF29:
	.string	"_IO_backup_base"
.LASF142:
	.string	"command"
.LASF110:
	.string	"__in6_u"
.LASF93:
	.string	"sin_addr"
.LASF65:
	.string	"__kind"
.LASF167:
	.string	"ftell"
.LASF2:
	.string	"long unsigned int"
.LASF118:
	.string	"ai_canonname"
.LASF216:
	.string	"chunks_available"
.LASF169:
	.string	"fopen"
.LASF241:
	.string	"chunk_sizes"
.LASF178:
	.string	"gettid"
.LASF59:
	.string	"__pthread_list_t"
.LASF122:
	.string	"long double"
.LASF74:
	.string	"uint16_t"
.LASF232:
	.string	"servers_contacted"
.LASF231:
	.string	"total_chunks_downloaded"
.LASF260:
	.string	"GNU C17 14.2.0 -mtune=generic -march=x86-64 -g -fasynchronous-unwind-tables -fstack-protector-strong -fstack-clash-protection -fcf-protection"
.LASF113:
	.string	"ai_family"
.LASF220:
	.string	"parse_server_filename"
.LASF77:
	.string	"SOCK_STREAM"
.LASF107:
	.string	"__u6_addr16"
.LASF238:
	.string	"total_bytes"
.LASF21:
	.string	"_IO_read_end"
.LASF200:
	.string	"server_info"
.LASF161:
	.string	"strchr"
.LASF96:
	.string	"sin6_family"
.LASF9:
	.string	"short int"
.LASF181:
	.string	"connect"
.LASF207:
	.string	"list_file"
.LASF12:
	.string	"long int"
.LASF204:
	.string	"hash"
.LASF256:
	.string	"length"
.LASF166:
	.string	"fread"
.LASF133:
	.string	"number_of_command"
.LASF218:
	.string	"temp"
.LASF51:
	.string	"_IO_wide_data"
.LASF224:
	.string	"original_len"
.LASF162:
	.string	"fclose"
.LASF217:
	.string	"can_rebuild"
.LASF146:
	.string	"connect_to_info"
.LASF182:
	.string	"socket"
.LASF208:
	.string	"file_info"
.LASF16:
	.string	"__ssize_t"
.LASF90:
	.string	"sockaddr_in"
.LASF195:
	.string	"EVP_DigestInit_ex"
.LASF8:
	.string	"__uint8_t"
.LASF69:
	.string	"__data"
.LASF84:
	.string	"SOCK_CLOEXEC"
.LASF26:
	.string	"_IO_buf_base"
.LASF64:
	.string	"__nusers"
.LASF42:
	.string	"_wide_data"
.LASF226:
	.string	"first_underscore"
.LASF101:
	.string	"__CONST_SOCKADDR_ARG"
.LASF39:
	.string	"_lock"
.LASF109:
	.string	"in6_addr"
.LASF141:
	.string	"sockfd"
.LASF125:
	.string	"EVP_MD_CTX"
.LASF50:
	.string	"_IO_codecvt"
.LASF35:
	.string	"_old_offset"
.LASF55:
	.string	"_IO_FILE"
.LASF79:
	.string	"SOCK_RAW"
.LASF72:
	.string	"pthread_mutex_t"
.LASF61:
	.string	"__lock"
.LASF120:
	.string	"__int128"
.LASF103:
	.string	"in_addr"
.LASF115:
	.string	"ai_protocol"
.LASF114:
	.string	"ai_socktype"
.LASF196:
	.string	"EVP_md5"
.LASF4:
	.string	"unsigned char"
.LASF209:
	.string	"chunks_present"
.LASF11:
	.string	"__uint32_t"
.LASF164:
	.string	"asprintf"
.LASF18:
	.string	"__socklen_t"
.LASF255:
	.string	"str2md5"
.LASF219:
	.string	"handle_req"
.LASF24:
	.string	"_IO_write_ptr"
.LASF81:
	.string	"SOCK_SEQPACKET"
.LASF253:
	.string	"cleanup_current_connection"
.LASF52:
	.string	"ssize_t"
.LASF148:
	.string	"number_of_servers"
.LASF203:
	.string	"recieve_buffer"
.LASF221:
	.string	"server_filename"
.LASF41:
	.string	"_codecvt"
.LASF139:
	.string	"dfsno"
.LASF137:
	.string	"server_port"
.LASF112:
	.string	"ai_flags"
.LASF163:
	.string	"fwrite"
.LASF124:
	.string	"evp_md_st"
.LASF13:
	.string	"__off_t"
.LASF171:
	.string	"recv"
.LASF149:
	.string	"number_of_available_servers"
.LASF6:
	.string	"signed char"
.LASF88:
	.string	"sa_family"
.LASF157:
	.string	"strdup"
.LASF5:
	.string	"short unsigned int"
.LASF165:
	.string	"memcpy"
.LASF138:
	.string	"server_ip"
.LASF187:
	.string	"atoi"
.LASF199:
	.string	"current"
.LASF85:
	.string	"SOCK_NONBLOCK"
.LASF20:
	.string	"_IO_read_ptr"
.LASF154:
	.string	"filename_length"
.LASF227:
	.string	"get_file"
.LASF121:
	.string	"__int128 unsigned"
.LASF71:
	.string	"__align"
.LASF32:
	.string	"_chain"
.LASF116:
	.string	"ai_addrlen"
.LASF134:
	.string	"commands_t"
.LASF48:
	.string	"FILE"
.LASF34:
	.string	"_flags2"
.LASF76:
	.string	"socklen_t"
.LASF248:
	.string	"server_index"
.LASF252:
	.string	"cleanup_connection"
.LASF70:
	.string	"__size"
.LASF189:
	.string	"freeaddrinfo"
.LASF36:
	.string	"_cur_column"
.LASF95:
	.string	"sockaddr_in6"
.LASF89:
	.string	"sa_data"
.LASF14:
	.string	"__off64_t"
.LASF47:
	.string	"_unused2"
.LASF80:
	.string	"SOCK_RDM"
.LASF60:
	.string	"__pthread_mutex_s"
	.section	.debug_line_str,"MS",@progbits,1
.LASF1:
	.string	"/home/parth/Work/All_data/university/Network System/Assignments/PA4/client"
.LASF0:
	.string	"src/handle_req.c"
	.ident	"GCC: (Ubuntu 14.2.0-4ubuntu2) 14.2.0"
	.section	.note.GNU-stack,"",@progbits
	.section	.note.gnu.property,"a"
	.align 8
	.long	1f - 0f
	.long	4f - 1f
	.long	5
0:
	.string	"GNU"
1:
	.align 8
	.long	0xc0000002
	.long	3f - 2f
2:
	.long	0x3
3:
	.align 8
4:
