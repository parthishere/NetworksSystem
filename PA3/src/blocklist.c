...
blocklist_t *init_blocklist(int new)
{
    ...
    blocklist = malloc(sizeof(blocklist_t));
    if (!new)
    ...
    blocklist->patterns = NULL;
    blocklist->pattern_count = 0;

    // Load patterns from file
    FILE *file = fopen(BLOCKLIST_FILE_NAME, "r");
    if (!file)
        return blocklist; // No blocklist file
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        blocklist->patterns = realloc(blocklist->patterns,
                                      (blocklist->pattern_count + 1) * sizeof(char *));
        blocklist->patterns[blocklist->pattern_count] = strdup(line);
        blocklist->pattern_count++;
    }
...
{
    if(bl == NULL) bl = _blocklist;

    for(int i=0; i < bl->pattern_count; i++){
        free(bl->patterns[i]);
        bl->patterns[i] = NULL;
    }
